export const tutorials =
{
  "def1": {
    "misconceptions": [
      "IsolatedFun"
    ],
    "order": [
      "intro_smol",
      "intro_defvar",
      "warmup_defvar",
      "intro_deffun_1",
      "warmup_deffun_1",
      "intro_deffun_2",
      "warmup_deffun_2",
      "intro_block",
      "intro_error",
      "warmup_error",
      "goal_undefined"
    ],
    "questions": {
      "intro_smol": {
        "confirm": "This series of tutorials revolve around a central idea, **SMoL**, the Standard Model of Languages.\nThis is the embodiment of the computational core of many of our widely-used programming languages,\nfrom C# and Java to JavaScript, and Python to Scala and Racket. All these languages (and many others),\nto a large extent, have a common computational core:\n\n- lexical scope\n- nested scope\n- eager evaluation\n- sequential evaluation (per “thread”)\n- mutable first-order variables\n- mutable first-class structures (objects, vectors, etc.)\n- higher-order functions that close over bindings\n- automated memory management (e.g., garbage collection)\n\nWhat goes in SMoL is, of course, a judgment call:\nwhen a feature isn't present across a large number of diverse languages (like static types),\nor shows too much variation between languages that have it (like objects),\nwe do not include that in the *standard* model.\nBut it is not a value judgment:\nthe standard model is about what languages *are*, rather than what languages *should be*.\n"
      },
      "intro_defvar": {
        "confirm": "In this tutorial, we will learn about **definitions**.\n\nThe following program illustrates **variable definitions**.\n\n```\n(defvar x 1)\n(defvar y 2)\nx\ny\n(+ x y)\n```\n\nIn this program, the first variable definition **binds** var`x` to `1`, and\nthe second variable definition binds var`y` to `2`.\n\nThis program produces three values:\nthe value of var`x`, the value of var`y`, and the value of term`(+ x y)`.\nThese values are `1`, `2`, and `3`, respectively.\nIn this Tutor, we will write the result of running this program on a single line as\n\n`1 2 3`\n\nrather than\n\n<code>1\n2\n3\n</code>\n"
      },
      "warmup_defvar": {
        "program": "(defvar x 1)\n(defvar y (+ x 2))\nx\ny\n",
        "answer": "1 3",
        "feedback": "The first definition binds var`x` to the value `1`.\nThe second definition binds var`y` to the value of term`(+ x 2)`, which is `3`.\nSo, the result of this program is `1 3`.\n",
        "again": {
          "program": "(defvar a 10)\n(defvar b (* 2 a))\na\nb\n",
          "answer": "10 20"
        }
      },
      "intro_deffun_1": {
        "confirm": "The following program illustrates **function definitions**.\n\n```\n(deffun (f x y)\n  (/ (+ x y) 2))\n\n(f (* 2 3) 4)\n```\n\nThis program produces `5`. It defines a function named var`f`,\nwhich has two **formal parameters**, var`x` and var`y`,\nand **calls** the function with the **actual parameters** `6` and `4`.\n"
      },
      "warmup_deffun_1": {
        "program": "(deffun (f x y z)\n  (+ x (+ y z)))\n(f 2 1 3)\n",
        "answer": "6",
        "feedback": "",
        "again": {
          "program": "(deffun (g x y z)\n  (* (* x y) z))\n(g 5 2 3)\n",
          "answer": "30"
        }
      },
      "intro_deffun_2": {
        "confirm": "Function definitions can contain definitions, for example\n\n```\n(deffun (f x y)\n  (defvar n (+ x y))\n  (/ n 2))\n\n(f (* 2 3) 4)\n```\n"
      },
      "warmup_deffun_2": {
        "program": "(deffun (f x y z)\n  (defvar p (* y z))\n  (+ x p))\n(f 2 1 3)\n",
        "answer": "5",
        "feedback": "",
        "again": {
          "program": "(deffun (f a b c)\n  (defvar q (+ b c))\n  (* a q))\n(f 3 2 1)\n",
          "answer": "9"
        }
      },
      "intro_block": {
        "emphasize": "We have two kinds of places where a definition might happen: the top-level **block** and\nfunction bodies (which are also **blocks**).\nA block is a sequence of definitions and expressions.\n\nBlocks form a tree-like structure in a program.\nFor example, we have four blocks in the following program:\n\n```\n(defvar n 42)\n\n(deffun (f x)\n  (defvar y 1)\n  (+ x y))\n\n(deffun (g)\n  (deffun (h m)\n    (* 2 m))\n  (f (h 3)))\n\n(g)\n```\n\nThe blocks are:\n\n- the top-level block, where the definitions of var`n`, var`f`, and var`g` appear\n- the body of var`f`, where the definition of var`y` appears, which is a sub-block of the top-level block\n- the body of var`g`, where the definition of var`h` appears, which is also a sub-block of the top-level block, and\n- the body of var`h`, where no local definition appears, which is a sub-block of the body of var`g`\n"
      },
      "intro_error": {
        "confirm": "We use the term **values** to refer to the typical result computations.\nThese include numbers, strings, booleans, etc.\nHowever, running a program can also produce an **error**.\n\nFor example, the result of the following program is `23 error`\nbecause you can't \"call\" a number, so the program stops executing as soon as it runs into an error.\n\n```\n(defvar x 23)\nx\n(x 0)\nx\n```\n"
      },
      "warmup_error": {
        "program": "(defvar xyz 42)\nabc\n",
        "answer": "error",
        "feedback": "The variable var`abc` is not defined.",
        "again": {
          "program": "(defvar foo 77)\nbar\n",
          "answer": "error"
        }
      },
      "goal_undefined": {
        "emphasize": "It is an error to evaluate an undefined variable.\n"
      }
    }
  },
  "def2": {
    "misconceptions": [
      "IsolatedFun",
      "FlatEnv"
    ],
    "order": [
      "intro_more_on_def",
      [
        "new_three_layer",
        "new_ref_global_and_local",
        "what_is_x_2",
        "error_when_refer_to_undefined",
        "shadow_rather_than_overwrite"
      ],
      "reflect_lexical_scope",
      "goal_lexical_scope",
      "goal_lexical_scope_labelling_1",
      "goal_lexical_scope_labelling_2",
      "goal_lexical_scope_labelling_3",
      "intro_scope",
      "scope_terminology_practice",
      "keyframe_lexical_scope_1",
      "keyframe_lexical_scope_2"
    ],
    "questions": {
      "intro_more_on_def": {
        "confirm": "In this tutorial, we will learn *more* about definitions.\n"
      },
      "new_three_layer": {
        "program": "(defvar x 1)\n(deffun (f y)\n  (deffun (g)\n    (defvar z 2)\n    (+ x y z))\n  (g))\n(+ (f 3) 4)\n",
        "answer": "10",
        "feedback": "This program binds var`x` to `1` and var`f` to a function and then\nevaluates term`(+ (f 3) 4)`.\nThe value of term`(+ (f 3) 4)` is the value of term`(+ (g) 4)`,\nwhich is the value of term`(+ (+ x y z) 4)`,\nwhich is the value of term`(+ (+ 1 3 2) 4)`,\nwhich is `10`.\n",
        "misconceptions": {
          "error": {
            "misconception": "IsolatedFun",
            "feedback": "You might think term`(+ x y z)` can't refer to var`x` and var`y`.\nHowever, term`(+ x y z)` appears in the body of var`g`, which is a sub-block of\nthe body of var`f`, which is a sub-block of the top-level block.\nSo, it can refer to the var`x` defined in the top-level block\nand the var`y` defined in var`f`.\nIn SMoL, variable references follow the hierarchical structure of blocks.\n"
          }
        },
        "again": {
          "program": "(defvar aa 3)\n(deffun (abc bb)\n  (deffun (h)\n    (defvar cc 2)\n    (* aa bb cc))\n  (h))\n(* (abc 4) 1)\n",
          "answer": "24"
        }
      },
      "new_ref_global_and_local": {
        "program": "(defvar x 1)\n(deffun (f)\n  (defvar y 2)\n  (deffun (g)\n    (+ x y))\n  (g))\n(f)\n",
        "answer": "3",
        "feedback": "term`(f)` returns the value of term`(g)`.\nThe value of term`(g)` is the value of term`(+ x y)`,\nwhich is term`(+ 1 2)`, which is `3`.\n",
        "misconceptions": {
          "error": {
            "misconception": "IsolatedFun",
            "feedback": "You might think term`(+ x y)` can't refer to var`x` and var`y`.\nHowever, term`(+ x y)` appears in the body of var`g`, which is a sub-block of\nthe body of var`f`.\nSo, it can refer to the var`x` defined in the top-level block and\nthe var`y` defined in var`f`.\nIn SMoL, variable references follow the hierarchical structure of blocks.\n"
          }
        },
        "again": {
          "program": "(defvar a 9)\n(deffun (fun)\n  (defvar b 2)\n  (deffun (prod)\n    (* a b))\n  (prod))\n(fun)\n",
          "answer": "18"
        }
      },
      "what_is_x_2": {
        "program": "(defvar x 1)\n(deffun (f)\n  x)\n(deffun (g)\n  (defvar x 2)\n  (f))\n\n(g)\n",
        "answer": "1",
        "feedback": "term`(g)` evaluates to term`(f)`, which evaluates to `1` because\nvar`f` and var`x` are both defined in the same block, and\nvar`f` does *not* get the var`x` defined inside var`g`.\n",
        "misconceptions": {
          "2": {
            "misconception": "FlatEnv",
            "feedback": "term`(defvar x 2)` binds var`x` to `2`.\nYou might think the binding applies everywhere.\nHowever, it only applies to the body of var`g`.\nThe var`x` in the body of var`f` refers to the var`x` defined in the top-level block.\nIn SMoL, variable references follow the hierarchical structure of blocks.\n"
          },
          "error": {
            "misconception": "IsolatedFun",
            "feedback": "You might think var`f` can't refer to var`x`.\nHowever, var`f` is defined in the top-level block.\nSo, it can refer to the var`x` defined in the top-level block.\nIn SMoL, variable references follow the hierarchical structure of blocks.\n"
          }
        },
        "again": {
          "program": "(defvar s 21)\n(deffun (i) s)\n(deffun (j)\n  (defvar s 76)\n  (i))\n\n(j)\n",
          "answer": "21"
        }
      },
      "error_when_refer_to_undefined": {
        "program": "(deffun (f x)\n  (defvar y 1)\n  (+ x y))\n\n(+ (f 2) y)\n",
        "answer": "error",
        "feedback": "term`(+ (f 2) y)` is evaluated in the top-level block, where var`y` is not defined.\nSo, this program errors.\n",
        "misconceptions": {
          "4": {
            "misconception": "FlatEnv",
            "feedback": "term`(defvar y 1)` binds var`y` to `1`.\nYou might think the binding applies everywhere.\nHowever, it only applies to the body of var`g`.\nterm`(+ (f 2) y)` appears in the top-level block, so it tries to refer to\na var`y` defined in the top-level block. But var`y` is not defined in the top-level block.\nIn SMoL, variable references follow the hierarchical structure of blocks.\n"
          }
        },
        "again": {
          "program": "(deffun (foo bar)\n  (defvar zzz 8)\n  (* bar zzz))\n\n(* (foo 9) zzz)\n",
          "answer": "error"
        }
      },
      "shadow_rather_than_overwrite": {
        "program": "(defvar x 1)\n(deffun (f y)\n  (defvar x 2)\n  (+ x y))\n\n(+ (f 0) x)\n",
        "answer": "3",
        "feedback": "This program binds var`x` to `1` and var`f` to a function in the top-level block. It binds var`x` to `2` in the body of var`f`.\nSo, term`(f 0)` evaluates to term`(+ 2 0)`, which is `2`, and term`(+ (f 0) x)` evaluates to term`(+ 2 1)`, which is `3`.\n",
        "misconceptions": {
          "4": {
            "misconception": "FlatEnv",
            "feedback": "term`(defvar x 2)` binds var`x` to `2`.\nYou might think the binding applies everywhere.\nHowever, it only applies to the body of var`f`.\nThe var`x` in term`(+ (f 0) x)` appears in the top-level block, so it refers to the var`x` defined in the top-level block.\nIn SMoL, variable references follow the hierarchical structure of blocks.\n"
          }
        },
        "again": {
          "program": "(defvar cat 7)\n(deffun (k dog)\n  (defvar cat 4)\n  (+ cat dog))\n\n(+ (k 1) cat)\n",
          "answer": "12"
        }
      },
      "reflect_lexical_scope": {
        "prompt": "When we see a variable reference (e.g., the var`x` in term`(+ x 1)`), how do we find its value, if any?\n"
      },
      "goal_lexical_scope": {
        "emphasize": "Variable references follow the hierarchical structure of blocks.\n\nIf the variable is defined in the current block, we use that declaration.\n\nOtherwise, we look up the block in which the current block appears, and so on recursively.\n(Specifically, if the current block is a function body, the next block will be the block in which the function definition is;\nif the current block is the top-level block, the next block will be the **primordial block**.)\n\nIf the current block is already the primordial block and we still haven't found a corresponding declaration,\nthe variable reference errors.\n\nThe primordial block is a non-visible block enclosing the top-level block.\nThis block defines values and functions that are provided by the language itself.\n"
      },
      "goal_lexical_scope_labelling_1": {
        "select_and_comment": "Now please scroll back and select 1-3 programs that make the point that\n\n> If the variable is declared in the current block, we use that declaration.\n\nYou don't need to select *all* such programs.\n"
      },
      "goal_lexical_scope_labelling_2": {
        "select_and_comment": "Please select 1-3 programs that make the point that\n\n> Otherwise, we look up the block in which the current block appears, and so on recursively.\n\nYou don't need to select *all* such programs.\n"
      },
      "goal_lexical_scope_labelling_3": {
        "select_and_comment": "Please select 1-3 programs that make the point that\n\n> If the current block is already the primordial block and we still haven't found a corresponding declaration,\n> the variable reference errors.\n\nYou don't need to select *all* such programs.\n"
      },
      "intro_scope": {
        "emphasize": "The **scope** of a variable is the region of a program where we can refer to the variable.\nTechnically, it includes the block in which the variable is defined (including sub-blocks)\n*except* the sub-blocks where the same name is re-defined.\nWhen the exception happens, that is, when the same name is defined in a sub-block,\nwe say that the variable in the sub-block **shadows** the variable in the outer block.\n\nWe say a variable reference (e.g., \"the var`x` in term`(+ x 1)`\") is **in the scope of** a declaration (e.g., \"the var`x` in term`(defvar x 23)`\")\nif and only if the former refers to the latter.\n"
      },
      "scope_terminology_practice": {
        "prompt": "Please select all statements that apply to the following program:\n\n```\n(defvar x 45)\n(deffun (f)\n  (deffun (g)\n    (+ x 1))\n  (defvar x 67)\n  (+ x 2))\n(f)\n(+ x 3)\n```\n",
        "checkboxes": [
          [
            false,
            "The scope of the top-level var`x` includes the whole program."
          ],
          [
            true,
            "The scope of the top-level var`x` includes term`(+ x 3)`."
          ],
          [
            false,
            "The top-level var`x` shadows the var`x` defined in var`f`."
          ],
          [
            true,
            "The var`x` in term`(+ x 1)` is in the scope of the var`x` defined in var`f`."
          ],
          [
            true,
            "The var`x` in term`(+ x 2)` is in the scope of the var`x` defined in var`f`."
          ],
          [
            false,
            "The var`x` in term`(+ x 3)` is in the scope of the var`x` defined in var`f`."
          ]
        ],
        "feedback": "The scope of the top-level var`x` includes the whole program *except* the body of var`f`.\nThe scope of the var`x` defined in var`f` is the body of var`f`.\n\nThe var`x` defined in var`f` shadows the top-level var`x` rather than the other way around.\n"
      },
      "keyframe_lexical_scope_1": {
        "prompt": "Here is a program that confused many students\n\n```\n(defvar x 1)\n(deffun (foo)\n  x)\n(deffun (bar)\n  (defvar x 2)\n  (foo))\n\n(bar)\n```\n\nPlease\n\n1. Run this program in the stacker by clicking the green run button above;\n2. The stacker would show how this program produces its result(s);\n3. Keep clicking <button disabled>⏭ Next</button> until you reach a configuration that you find particularly helpful;\n4. Click <button disabled>🔗 Share This Configuration</button> to get a link to your configuration;\n5. Submit your link below;\n"
      },
      "keyframe_lexical_scope_2": {
        "prompt": "Please write a couple of sentences to explain how your configuration explains the result(s) of the program.\n"
      }
    }
  },
  "def3": {
    "misconceptions": [
      "Lazy"
    ],
    "order": [
      "intro_even_more_on_def",
      [
        "defvar_binding_cause_evaluation",
        "funcall_binding_cause_evaluation",
        "bad_order"
      ],
      "reflect_order_of_computation",
      "goal_order_of_computation",
      "goal_order_of_computation_labelling_1",
      "goal_order_of_computation_labelling_2",
      "keyframe_order_of_computation_1",
      "keyframe_order_of_computation_2"
    ],
    "questions": {
      "intro_even_more_on_def": {
        "confirm": "In this tutorial, we will learn even more about definitions.\n"
      },
      "defvar_binding_cause_evaluation": {
        "program": "(defvar x (12 54))\n3\n",
        "answer": "error",
        "feedback": "When you define a variable (in this case, var`x`), you have to bind it to a value, no\nmatter whether or not you need the value of that variable later in the program.\nThe program errors when it tries to evaluate term`(12 54)`.\n",
        "misconceptions": {
          "3": {
            "misconception": "Lazy",
            "feedback": "You might think term`(12 54)` is not evaluated because var`x` is not needed.\nHowever, this is not the case.\nIn SMoL, every variable definition evaluates the expression immediately and binds the variable to the value, even if the variable is not used later in the program.\n"
          }
        },
        "again": {
          "program": "(defvar b (79 6))\n1\n",
          "answer": "error"
        }
      },
      "funcall_binding_cause_evaluation": {
        "program": "(deffun (f x)\n  3)\n(f (7 5))\n",
        "answer": "error",
        "feedback": "Function calls bind their formal parameters\n(in this case, there is one formal parameter, var`x`)\nto the values of actual parameters\n(in this case, there is one actual parameter, term`(7 5)`).\nThe program errors when it tries to evaluate term`(7 5)`.\n",
        "misconceptions": {
          "3": {
            "misconception": "Lazy",
            "feedback": "You might think term`(7 5)` is not evaluated because var`x` is not needed in the function.\nHowever, this is not the case.\nIn SMoL, every function call evaluates the actual parameters immediately and binds the values to formal parameters, even when the formal parameter is not used in the function.\n"
          }
        },
        "again": {
          "program": "(deffun (z y)\n  5)\n(z (78 92))\n",
          "answer": "error"
        }
      },
      "bad_order": {
        "program": "(defvar x (+ y 1))\n(defvar y 2)\n\nx\ny\n",
        "answer": "error",
        "feedback": "The first definition tries to bind var`x` to the value of term`(+ y 1)`.\nTo evaluate term`(+ y 1)`, we need the value of var`y`.\nBut var`y` is not bound to a value at that moment.\n",
        "misconceptions": {
          "3 2": {
            "misconception": "Lazy",
            "feedback": "You might think term`(+ y 1)` is able to refer to var`y` and hence evaluate to a value.\nHowever, var`y` has not been bound to a value when term`(+ y 1)` is evaluated.\nIn SMoL, every block evaluates its definitions and expressions in reading order (i.e., top-to-bottom and left-to-right).\n"
          }
        },
        "again": {
          "program": "(defvar baz (+ bar 1))\n(defvar bar 2)\n\nbaz\nbar\n",
          "answer": "error"
        }
      },
      "reflect_order_of_computation": {
        "prompt": "In what order are definitions and expressions evaluated?\n"
      },
      "goal_order_of_computation": {
        "emphasize": "- Variables are bound to values. Specifically, every variable definition evaluates the expression immediately and binds the variable to the value, even if the variable is not used later in the program; every function call evaluates the actual parameters immediately and binds the values to formal parameters, even if the formal parameter is not used in the function.\n- Every block evaluates its definitions and expressions in reading order (i.e., top-to-bottom and left-to-right).\n"
      },
      "goal_order_of_computation_labelling_1": {
        "select_and_comment": "Please scroll back and select 1-3 programs that together make these points.\n\n> - Variables are bound to values. Specifically, every variable definition evaluates the expression immediately and binds the variable to the value, even if the variable is not used later in the program; every function call evaluates the actual parameters immediately and binds the values to formal parameters, even if the formal parameter is not used in the function.\n\nYou don't need to select all such programs.\n"
      },
      "goal_order_of_computation_labelling_2": {
        "select_and_comment": "Please scroll back and select 1-3 programs that together make these points.\n\n> - Every block evaluates in top-to-bottom, left-to-right order.\n\nYou don't need to select all such programs.\n"
      },
      "keyframe_order_of_computation_1": {
        "prompt": "Here is a program that confused many students\n\n```\n(deffun (addy x)\n  (+ x y))\n(defvar s (addy 1))\n(defvar y 2)\n\ns\n```\n\nPlease\n\n1. Run this program in the stacker by clicking the green run button above;\n2. The stacker would show how this program produces its result(s);\n3. Keep clicking <button disabled>⏭ Next</button> until you reach a configuration that you find particularly helpful;\n4. Click <button disabled>🔗 Share This Configuration</button> to get a link to your configuration;\n5. Submit your link below;\n"
      },
      "keyframe_order_of_computation_2": {
        "prompt": "Please write a couple of sentences to explain how your configuration explains the result(s) of the program.\n"
      }
    }
  },
  "vectors1": {
    "misconceptions": [],
    "order": [
      "intro_vectors",
      "warmup_mvec",
      "intro_vecref",
      "warmup_vecref",
      "intro_vecset",
      "warmup_vecset"
    ],
    "questions": {
      "intro_vectors": {
        "confirm": "In this tutorial, we will learn about **mutable values**, illustrated with **vectors**.\n\nThe following program illustrates how to create vectors\n\n```\n(mvec (mvec 1 2) (mvec 3) (mvec))\n```\n\nThis program produces `#(#(1 2) #(3) #())`. It creates four vectors:\n\n* a two-**element** vector that refers `1` and `2`\n* a one-element vector that refers `3`\n* an empty vector\n* a three-element vector that refers all three aforementioned vectors\n"
      },
      "warmup_mvec": {
        "program": "(defvar n 44)\n(deffun (f x)\n  (+ x 1))\n(mvec n (f n))\n",
        "answer": "#(44 45)",
        "feedback": "This program binds var`n` to `44` and var`f` to a function that adds 1 to its parameter.\nAfter that, the program creates a vector that refers the value of var`n`,\nwhich is `44`,\nand the value of term`(f n)`,\nwhich is `45`.\nThe vector is printed as `#(44 45)`.\n",
        "again": {
          "program": "(defvar x 7)\n(deffun (fun n)\n  (* n 2))\n(mvec (fun x) x)\n",
          "answer": "#(14 7)"
        }
      },
      "intro_vecref": {
        "confirm": "The following program illustrates how to refer to vector elements.\n\n```\n(defvar v (mvec 84 75))\n(vec-ref v 0)\n(vec-ref v 1)\n(vec-ref v 2)\n```\n\nThis program produces `84 75 error`.\nIt refers to the `0`-th (i.e., first) element, the `1`-th element, and then\ntries to refer to the `2`-th element.\n"
      },
      "warmup_vecref": {
        "program": "(defvar v (mvec (mvec 58 43) (mvec 43 66)))\n(defvar vr (vec-ref v 1))\n(vec-ref vr 0)\n",
        "answer": "43",
        "feedback": "Recall that the left-most element is the `0`-th (rather than 1-th!) element.\nterm`(vec-ref v 1)` produces the value of term`(mvec 43 66)`.\nSo, term`(vec-ref vr 0)` produces `43`.\n",
        "misconceptions": {
          "58": {
            "misconception": "Other"
          }
        },
        "again": {
          "program": "(defvar v (mvec (mvec 81 82 83) (mvec 84 85 86) (mvec 87 88 89)))\n(vec-ref (vec-ref v 2) 1)\n",
          "answer": "88"
        }
      },
      "intro_vecset": {
        "confirm": "The following program illustrates how to mutate vectors.\n\n```\n(defvar m (mvec 62 77))\n(vec-set! m 0 83)\n(vec-ref m 0)\n```\n\nThis program produces `83`.\nIt **mutates** the vector by **replacing** the `0`-th element with `83`\nand then refers to the `0`-th element.\n\n"
      },
      "warmup_vecset": {
        "program": "(defvar x (mvec 92 73))\n(vec-set! x 0 67)\nx\n",
        "answer": "#(67 73)",
        "feedback": "var`x` is bound to a vector.\nThe initial content of the vector is `92` and `73`.\nThe mutation replaces the first element with `67`.\nA vector referring `67` and `73` is printed as `#(67 73)`.\n",
        "misconceptions": {
          "67 73": {
            "misconception": "Other",
            "feedback": "var`x` is bound to a vector.\nA vector referring `67` and `73` is printed as `#(67 73)`.\n"
          }
        },
        "again": {
          "program": "(defvar v (mvec 83 64))\n(vec-set! v 0 66)\nv\n",
          "answer": "#(66 64)"
        }
      }
    }
  },
  "vectors2": {
    "misconceptions": [
      "DefCopyStruct",
      "CallCopyStruct",
      "StructCopyStruct",
      "NoCircularity"
    ],
    "order": [
      "intro_more_on_vectors",
      [
        "alias_with_defvar",
        "new_alias_with_defvar",
        "alias_with_funcall",
        "new_alias_with_funcall",
        "alias_mvec_in_mvec",
        "alias_mvec_in_mpair",
        "basic_circularity",
        "new_circularity"
      ],
      "reflect_vectors",
      "goal_vectors",
      "goal_vectors_labelling",
      "reflect_heap",
      "goal_heap",
      "comprehension_check1",
      "comprehension_check2",
      "reflect_heap_not_env",
      "goal_heap_not_env",
      "keyframe_vector_aliasing_1",
      "keyframe_vector_aliasing_2"
    ],
    "questions": {
      "intro_more_on_vectors": {
        "confirm": "In this tutorial, we will learn *more* about **mutable values**, illustrated with **vectors**.\n"
      },
      "alias_with_defvar": {
        "program": "(defvar x (mvec 52 70 61))\n(defvar y x)\n(vec-set! x 0 93)\n(vec-ref y 0)\n",
        "answer": "93",
        "feedback": "The program creates a vector where the `0`-th element is `52`.\nand binds it to both var`x` and var`y`.\nThe `0`-th element of the vector is then replaced with `93`.\nSo, the result is `93`.\n",
        "misconceptions": {
          "52": {
            "misconception": "DefCopyStruct",
            "feedback": "You might think var`x` and var`y` refer to different vectors,\nso changing var`x` doesn't change var`y`.\nHowever, they refer to the same vector.\nIn SMoL, a vector can be referred to by more than one variable,\nand binding a vector to a new variable does not create a copy of that vector.\n"
          }
        },
        "again": {
          "program": "(defvar a (mvec 37 26))\n(defvar b a)\n(vec-set! a 0 87)\nb\n",
          "answer": "#(87 26)"
        }
      },
      "new_alias_with_defvar": {
        "program": "(defvar x (mvec 62))\n(defvar y x)\n(vec-set! y 0 34)\nx\n",
        "answer": "#(34)",
        "feedback": "var`x` is first bound to a vector that refers `62`. var`y` is bound to the same vector.\nThe vector mutation replaces `62` with `34`. So, eventually, the vector becomes `#(34)`.\n",
        "misconceptions": {
          "#(62)": {
            "misconception": "DefCopyStruct",
            "feedback": "You might think var`x` and var`y` refer to different vectors, so changing var`y` doesn't change var`x`.\nHowever, they refer to the same vector.\nIn SMoL, a vector can be referred to by more than one variable, and binding a vector to a new variable does not create a copy of that vector.\n"
          }
        },
        "again": {
          "program": "(defvar foo (mvec 65 48))\n(defvar bar foo)\n(vec-set! bar 0 55)\nfoo\n",
          "answer": "#(55 48)"
        }
      },
      "alias_with_funcall": {
        "program": "(defvar x (mvec 10 48 95))\n(deffun (f y)\n  (vec-set! y 0 32)\n  (vec-ref y 0))\n(defvar z (f x))\n(vec-ref x 0)\n",
        "answer": "32",
        "feedback": "The program first creates a three-element vector, where the `0`-th element is `10`.\nAfter that, the program defines a function var`f`.\nThe function call term`(f x)` replaces the `0`-th vector element with `32`.\nAfter that, the vector is printed. So the\nresult is `32`.\n",
        "misconceptions": {
          "71": {
            "misconception": "CallCopyStruct",
            "feedback": "You might think var`x` and var`y` refer to different vectors, so changing var`y` doesn't change var`x`.\nHowever, they refer to the same vector.\nIn SMoL, a vector can be referred to by more than one variable, and vectors that are passed to a function in a function call do not get copied..\n"
          }
        },
        "again": {
          "program": "(defvar a (mvec 55 17))\n(deffun (foobar b)\n  (vec-set! b 0 52))\n(foobar a)\na\n",
          "answer": "#(52 17)"
        }
      },
      "new_alias_with_funcall": {
        "program": "(defvar x (mvec 99 83))\n(deffun (f y)\n  (vec-set! x 0 34)\n  y)\n(f x)\n",
        "answer": "#(34 83)",
        "feedback": "The program creates a vector and binds it to var`x`.\nThe function call term`(f x)` binds var`y` to the same vector.\nThe function replaces the `0`-th element of the vector with `34`\nand then returns the vector.\nSo, the final result is the vector, which is printed as `#(34 83)`.\n",
        "misconceptions": {
          "#(99 83)": {
            "misconception": "CallCopyStruct",
            "feedback": "You might think var`x` and var`y` refer to different vectors, so changing var`x` doesn't change var`y`.\nHowever, they refer to the same vector.\nIn SMoL, a vector can be referred to by more than one variable, and vectors that are passed to a function in a function call do not get copied..\n"
          },
          "error": {
            "misconception": "IsolatedFun",
            "feedback": "You might think term`(vec-set! x 0 34)` can't refer to var`x`.\nHowever, term`(vec-set! x 0 34)` appears in the body of var`f`, which is a sub-block of the top-level block.\nSo, it can refer to the var`x` defined in the top-level block.\nIn SMoL, variable references follow the hierarchical structure of blocks.\n"
          }
        },
        "again": {
          "program": "(defvar a (mvec 66 54))\n(deffun (h b)\n  (vec-set! a 0 42)\n  b)\n(h a)\n",
          "answer": "#(42 54)"
        }
      },
      "alias_mvec_in_mvec": {
        "program": "(defvar x (mvec 53))\n(defvar v (mvec 72 x))\n(vec-set! x 0 72)\nv\n",
        "answer": "#(72 #(72))",
        "feedback": "var`x` is bound to a one-element vector.\nThe only element of this vector is `53`.\nvar`v` is bound to a two-element vector.\nThe elements of this vector are `72` and the one-element vector.\nThe subsequent term`(vec-set! x 0 72)` mutates the one-element vector.\nFinally, the value of var`v` is printed.\n",
        "misconceptions": {
          "#(72 #(53))": {
            "misconception": "StructCopyStruct",
            "feedback": "You might think var`x` and the 1-th element of var`v` refer to different vectors, so changing var`x` doesn't change the 1-th element of var`v`.\nHowever, they refer to the same vector.\nIn SMoL, a vector can be referred to by multiple places, and creating new vectors that refer to existing ones does not create new copies of the existing vectors.\n"
          }
        },
        "again": {
          "program": "(defvar m (mvec 0))\n(defvar v (mvec 52 m 53))\n(vec-set! m 0 71)\nv\n",
          "answer": "#(52 #(71) 53)"
        }
      },
      "alias_mvec_in_mpair": {
        "program": "(defvar x (mvec 19 73 28))\n(defvar y (mvec x x))\n(vec-set! (vec-ref y 0) 0 64)\n(vec-ref y 1)\n",
        "answer": "#(64 73 28)",
        "feedback": "This program first creates a three-element vector and binds it to var`v`.\nAfter that, the program creates a two-element vector. Both elements refer to the first vector.\nAfter that, the `0`-th element of the three-element vector is replaced with `64`.\nFinally, the three-element vector is printed.\n",
        "misconceptions": {
          "#(51 73 28)": {
            "misconception": "StructCopyStruct",
            "feedback": "You might think the two elements of var`v` refer to different vectors, so changing one doesn't change the other.\nHowever, they refer to the same vector.\nIn SMoL, a vector can be referred to by multiple places, and creating new vectors that refer to existing ones does not create new copies of the existing vectors.\n"
          }
        },
        "again": {
          "program": "(defvar mv (mvec 63))\n(defvar mv2 (mvec mv mv))\n(vec-set! (vec-ref mv2 0) 0 42)\n(vec-ref mv2 1)\n",
          "answer": "#(42)"
        }
      },
      "basic_circularity": {
        "program": "(defvar x (mvec 43 54))\n(vec-set! x 0 x)\n(vec-ref x 1)\n",
        "answer": "54",
        "feedback": "var`x` is bound to a vector.\nterm`(vec-set! x 0 x)` makes the vector refer to itself.\nThis is fine because a vector element can be any value, including itself.\nBesides, the vector is not copied, so the mutation finishes immediately.\n",
        "misconceptions": {
          "error": {
            "misconception": "NoCircularity",
            "feedback": "You might think var`x` can't refer to itself.\nHowever, this is not the case.\nIn SMoL, vectors can be referred to by arbitrary vectors, including themselves.\n"
          }
        },
        "again": {
          "program": "(defvar x (mvec 84 73 69 52))\n(vec-set! x 1 x)\n(vec-ref x 0)\n",
          "answer": "84"
        }
      },
      "new_circularity": {
        "program": "(defvar x (mvec 74 82))\n(defvar y (mvec x))\n(vec-set! x 0 y)\n(vec-ref x 1)\n",
        "answer": "82",
        "feedback": "The program binds var`x` to a two-element vector and var`y` to a one-element vector.\nAfter that, it replaces the `0`-th element of the two-element vector with the one-element vector.\nThe other element of the two-element vector is still `82`.\n",
        "misconceptions": {
          "error": {
            "misconception": "NoCircularity",
            "feedback": "You might think var`x` and var`y` can't form a circle.\nHowever, this is not the case.\nIn SMoL, vectors can be referred to by arbitrary vectors, including themselves.\n"
          }
        },
        "again": {
          "program": "(defvar a (mvec 41 92))\n(defvar b (mvec a))\n(vec-set! a 1 b)\n(vec-ref a 0)\n",
          "answer": "41"
        }
      },
      "reflect_vectors": {
        "prompt": "What did you learn about vectors from these programs?"
      },
      "goal_vectors": {
        "emphasize": "A vector can be referred to by more than one variable and even by other vectors (including itself).\nReferring to a vector does not create a copy of the vector; rather, they share the same vector. Specifically\n\n- Binding a vector to a new variable does not create a copy of that vector.\n- Vectors that are passed to a function in a function call do not get copied.\n- Creating new vectors that refer to existing ones does not create new copies of the existing vectors.\n\nThe references share the same vector. That is, vectors can be **aliased**.\n"
      },
      "goal_vectors_labelling": {
        "select_and_comment": "Now please scroll back and select 1-3 programs that make the above points.\n\nYou don't need to select *all* such programs.\n"
      },
      "reflect_heap": {
        "prompt": "Reconsider these two programs that you might have seen. The only difference is in term`(defvar y ___)`.\n\n```\n(defvar x (mvec 52 96))\n(defvar y x)\n(vec-set! x 0 34)\ny\n```\n\n```\n(defvar x (mvec 52 96))\n(defvar y (mvec 52 96))\n(vec-set! x 0 34)\ny\n```\n\nIt is tempting to describe the variables as\n\n- var`x` is bound to `#(52 96)`\n- var`y` is bound to `#(52 96)`\n\nThis description does not help us to understand the program\nbecause it can't explain why term`(vec-set! x 0 34)` mutates var`y` in one case,\nand does not in the other.\n\nWhat is a better way to describe the bindings that help solve this problem?\n",
        "feedback": "We can say, in the first program\n\n- var`x` is bound to `@100`\n- var`y` is bound to `@100`\n\nwhere\n\n- `@100` is `#(52 96)`\n\nWhile for the other program\n\n- var`x` is bound to `@100`\n- var`y` is bound to `@200`\n\nwhere\n\n- `@100` is `#(52 96)`\n- `@200` is `#(52 96)`\n"
      },
      "goal_heap": {
        "emphasize": "In SMoL, each vector has its own unique **heap address** (e.g., `@100` and `@200`).\nThe mapping from addresses to vectors is called the **heap**.\n\n(**Note**: we use `@ddd` (e.g., `@123`, `@200`, and `@100`) to represent heap addresses.\nHeap addresses are *random*. The numbers don't mean anything.)\n"
      },
      "comprehension_check1": {
        "prompt": "Which choice best describes the status of the heap at the end of the following program?",
        "program": "(defvar x 3)\n(defvar v (mvec 1 2 x))\n",
        "distractors": [
          "@1 = #(1 2 x)",
          "There is nothing in the heap."
        ],
        "answer": "@1 = #(1 2 3)",
        "feedback": "%2% is wrong because the term`(mvec 1 2 x)` creates a vector.\nEvery vector is stored on the heap.\n\n%1% is wrong because vectors refer values, while var`x` is not a value.\n"
      },
      "comprehension_check2": {
        "prompt": "Which choice best describes the status of the heap at the end of the following program?",
        "program": "(defvar mv (mvec 3))\n(defvar mv2 (mvec mv mv))\n(vec-set! (vec-ref mv2 0) 0 42)\n",
        "distractors": [
          "@100 = #(3); @200 = #(@100 @100)",
          "@100 = #(42); @200 = #(#(42) #(42))",
          "@100 = #(3); @200 = #(#(42) #(3))",
          "@100 = #(3); @200 = #(@300 @100); @300 = #(42)",
          "There is nothing in the heap."
        ],
        "answer": "@100 = #(42); @200 = #(@100 @100)",
        "feedback": "%2% and %3% are wrong.\nVectors refer values.\n`#(42)` and `#(3)` are not values, although some vector values are printed like them.\nSo, `@200 = #(#(42) #(42))` and `@200 = #(#(42) #(3))` can not be valid.\n\nterm`(mvec 3)` creates a 1-element vector, `@100`.\nThe only element is `3`.\nterm`(mvec mv mv)` creates a 2-element vector, `@200`.\nBoth elements of `@200` are `@100`.\nNo more vectors are created, which means %4% must be wrong.\nSo, then, the correct answer must be %1% or %0%.\n\nHowever, the subsequent mutation changes `@100` (the first element of `@200`).\nThe `0`-th element of `@100` is mutated to `42`.\nSo, %0% is the correct answer.\n"
      },
      "reflect_heap_not_env": {
        "prompt": "The following program defines two variables but\ncreates nothing on the heap.\n\n```\n(defvar x 2)\n(defvar y 3)\nx\ny\n```\n\nThe following program defines no variables but\ncreates two things on the heap.\n\n```\n(mvec 1 (mvec 2 3))\n```\n\nWhat did you learn from this pair of programs?\n"
      },
      "goal_heap_not_env": {
        "emphasize": "- Creating a vector does not inherently create a binding.\n- Creating a binding does not necessarily alter the heap.\n"
      },
      "keyframe_vector_aliasing_1": {
        "prompt": "Here is a program that confused many students\n\n```\n(defvar v (mvec 1 2 3 4))\n(defvar vv (mvec v v))\n(vec-set! (vec-ref vv 1) 0 100)\nvv\n```\n\nPlease\n\n1. Run this program in the stacker by clicking the green run button above;\n2. The stacker would show how this program produces its result(s);\n3. Keep clicking <button disabled>⏭ Next</button> until you reach a configuration that you find particularly helpful;\n4. Click <button disabled>🔗 Share This Configuration</button> to get a link to your configuration;\n5. Submit your link below;\n"
      },
      "keyframe_vector_aliasing_2": {
        "prompt": "Please write a couple of sentences to explain how your configuration explains the result(s) of the program.\n"
      }
    }
  },
  "vectors3": {
    "misconceptions": [],
    "order": [
      "intro_even_more_on_mvec",
      "warmup_vector",
      [
        "mvecs_are_mvec",
        "vector_not_flatten",
        "alias_mvec_in_mvec",
        "new_alias_mvec_in_mvec",
        "basic_circularity"
      ]
    ],
    "questions": {
      "intro_even_more_on_mvec": {
        "confirm": "In this tutorial, we will learn even more about **mutable values**, illustrated with **vectors**.\n"
      },
      "warmup_vector": {
        "prompt": "Which choice best describes the heap at the end of the following program?\n\n(**Note**: we use `@ddd` (e.g., `@123`, `@200`, and `@100`) to represent heap addresses.\nHeap addresses are *random*. The numbers don't mean anything.)\n",
        "program": "(defvar x (mvec 2))\n(vec-set! x 0 33)\nx\n",
        "distractors": [
          "@200 = #(2)",
          "@200 = 33",
          "@100 = #(2); @200 = #(33)"
        ],
        "answer": "@200 = #(33)",
        "feedback": "Exactly one vector was created.\nSo, there must be at most one vector on the heap.\nThis rules out %3%.\n\nFor now, the heap maps addresses only to vectors.\nThis rules out %2%.\n\nThe heap looks like %1% after evaluating term`(mvec 2)`.\nHowever, the subsequent mutation changes the vector.\nSo, the correct answer is %0%.\n"
      },
      "mvecs_are_mvec": {
        "prompt": "Which choice best describes the heap at the end of the following program?\n",
        "program": "(defvar mv (mvec 4 5))\n(vec-ref mv 0)\n",
        "distractors": [
          "@100 = #(4 5); @200 = 4",
          "@100 = #(@200 @300); @200 = 4; @300 = 5"
        ],
        "answer": "@100 = #(4 5)",
        "feedback": "For now, the heap maps addresses only to vectors.\nThis rules out %1,2%.\n\nSo, %0% is correct.\n"
      },
      "vector_not_flatten": {
        "prompt": "Which choice best describes the heap at the end of the following program?\n",
        "program": "(defvar m (mvec 1 2))\n(vec-set! m 1 (mvec 3 4))\n",
        "distractors": [
          "@100 = #(1 2)",
          "@100 = #(1 2); @200 = #(3 4)",
          "@100 = #(@200 1); @200 = #(3 4)",
          "@100 = #(1 #(3 4)); @200 = #(3 4)",
          "@100 = #(1 #(3 4))"
        ],
        "answer": "@100 = #(1 @200); @200 = #(3 4)",
        "feedback": "Two vectors are created. So, there must be at least two things on the heap.\nThis rules out %1,5%.\n\n%4% is wrong because vectors refer values (e.g., `1`, `2`, and `@200`).\n`#(3 4)` is not itself a value; it's the *printing* of the value that resides at `@200`.\n%2% can be the heap after the second vector is created.\nHowever, the subsequent mutation changes `@100`.\nSo, %2% is wrong.\n\nThe mutation replaces the 1-th (i.e., second) element rather than the `0`-th element,\nso %0% is correct, while %3% is wrong.\n"
      },
      "alias_mvec_in_mvec": {
        "prompt": "Which choice best describes the heap at the end of the following program?\n",
        "program": "(defvar x (mvec 3))\n(defvar v (mvec 1 2 x))\n(vec-set! x 0 4)\nv\n",
        "distractors": [
          "@100 = #(4); @200 = #(1 2 #(3))",
          "@100 = #(4); @200 = #(1 2 #(4))",
          "@100 = #(4); @200 = #(1 2 3)",
          "@100 = #(4); @200 = #(1 2 4)",
          "@100 = #(3); @200 = #(1 2 @300); @300 = #(4)",
          "@100 = #(3); @200 = #(1 2 @100)"
        ],
        "answer": "@100 = #(4); @200 = #(1 2 @100)",
        "feedback": "%1,2% are wrong because vectors refer values.\n`#(3)` and `#(4)` are not values, although\nthey can be the printed representation of a vector.\n\n%3,4% are wrong because the 2-th element of the 3-element vector must be a vector.\nThe 3-element vector is created by term`(mvec 1 2 x)`.\nThe value of var`x` is a vector at that moment.\nThis 3-element vector is never mutated.\n\n%5% is wrong because only two vectors are created.\nThere must be at most two vectors on the heap.\n\n%6% can be the heap after the two vectors are created.\nHowever, the subsequent mutation changes the shorter vector.\nSo, %0% is the correct answer.\n"
      },
      "new_alias_mvec_in_mvec": {
        "prompt": "Which choice best describes the heap at the end of the following program?\n",
        "program": "(defvar v (mvec 1 2 3))\n(defvar vv (mvec v v))\n(vec-set! (vec-ref vv 1) 0 6)\n(vec-ref vv 0)\n",
        "distractors": [
          "@100 = #(6 2 3); @200 = #(#(1 2 3) #(6 2 3))",
          "@100 = #(1 2 3); @200 = #(#(1 2 3) #(6 2 3))",
          "@100 = #(1 2 3); @200 = #(@100 @300); @300 = #(6 2 3)",
          "@100 = #(1 2 3); @200 = #(1 2 3); @300 = #(6 2 3); @400 = #(@200 @300)",
          "@100 = #(1 2 3); @200 = #(@100 @100)",
          "@100 = #(1 2 3); @200 = #(6 @100)"
        ],
        "answer": "@100 = #(6 2 3); @200 = #(@100 @100)",
        "feedback": "Vectors refer values (e.g., `1` and `@200`).\nThis rules out %1,2%.\n\nTwo vectors are created.\nSo, there must be two vectors on the heap.\nThis rules out %3,4%.\n\nvar`vv` is bound to a 2-element vector, and the\n1-th element of the 2-element vector must be the 3-element vector.\nThe mutation replaces the `0`-th element in the 3-element vector with `6`.\nSo, %0% is the correct answer, while\n%5% does not reflect the effect of the mutation, and %6% mutates the wrong vector.\n"
      },
      "basic_circularity": {
        "prompt": "Which choice best describes the heap at the end of the following program?\n",
        "program": "(defvar x (mvec 1 0 2))\n(vec-set! x 1 x)\n(vec-len x)\n",
        "distractors": [
          "@100 = #(1 0 2)",
          "@100 = #(1 @200 2); @200 = #(1 0 2)",
          "@100 = #(1 #(1 0 2) 2)"
        ],
        "answer": "@100 = #(1 @100 2)",
        "feedback": "%3% is wrong because vectors refer values.\n`#(1 0 2)` is not a value, although\nit can be some vector values printed.\n\n%2% is wrong because only one vector is created.\nThere must not be two vectors on the heap.\n\n%1% can be the heap after the vector is created.\nBut the subsequent mutation replaces the 1-th element of `@100` with `@100`.\nSo, %0% is the correct answer.\n"
      }
    }
  },
  "mutvars1": {
    "misconceptions": [
      "DefByRef",
      "CallByRef",
      "StructByRef",
      "IsolatedFun"
    ],
    "order": [
      "intro_mutvars",
      "warmup_set",
      [
        "not_aliased_by_defvar_1",
        "not_aliased_by_defvar_2",
        "not_aliased_by_funarg_2",
        "new_not_aliased_by_funarg",
        "alias_var_in_mvec",
        "new_alias_mvec_in_mvec_trick"
      ],
      "reflect_mutvars_1",
      "goal_mutvars_1",
      "goal_mutvars_1_labelling"
    ],
    "questions": {
      "intro_mutvars": {
        "confirm": "In this tutorial, we will learn about **variable assignments** and **mutable variables**.\n\nThe following program illustrates the new concepts.\n\n```\n(defvar x 2)\nx\n(set! x (+ x 1))\nx\n(set! x (* x 2))\nx\n```\n\nThis program produces `2 3 6`.\nIt first defines var`x` and binds var`x` to `2`.\nThe first variable assignment term`(set! x (+ x 1))` **mutates** (the binding of) var`x`.\nAfter that, var`x` is bound to the value of term`(+ x 1)`; this uses the new value of var`x`, which is `2`, resulting in term`(+ 2 1)`, which is `3`.\nThe next variable assignment again mutates var`x` and binds var`x` to the value of term`(* 3 2)`, which is `6`.\n"
      },
      "warmup_set": {
        "program": "(defvar rent 10)\n(set! rent (* 10 2))\nrent\n",
        "answer": "20",
        "feedback": "var`rent` is bound to `10` initially.\nThe `set!` mutates var`rent` to `20`.\nSo, when we print the value of var`rent` after the `set!`,\nwe see `20`.\n",
        "again": {
          "program": "(defvar discount 10)\n(set! discount (+ 10 15))\ndiscount\n",
          "answer": "25"
        }
      },
      "not_aliased_by_defvar_1": {
        "program": "(defvar x 80)\n(defvar y x)\n(set! x 32)\nx\ny\n",
        "answer": "32 80",
        "feedback": "The first definition binds var`x` to `80`.\nThe second definition binds var`y` to the value of var`x`, which is `80`.\nThe variable assignment mutates the binding of var`x`, so var`x` is now bound to `32`.\nBut var`y` is still bound to `80`.\n",
        "misconceptions": {
          "32 32": {
            "misconception": "DefByRef",
            "feedback": "You might think term`(defvar y x)` binds var`y` to var`x`, so changing var`x` will change var`y`.\nHowever, we learned in previous tutorials that variables are bound to values, so var`y` is bound to the `80`, i.e., *the value of* var`x`.\nIn SMoL, variable assignments change *only* the mutated variables.\n"
          }
        },
        "again": {
          "program": "(defvar a 1)\n(defvar b a)\n(set! a 2)\na\nb\n",
          "answer": "2 1"
        }
      },
      "not_aliased_by_defvar_2": {
        "program": "(defvar m 40)\n(defvar n m)\n(set! n 22)\nm\nn\n",
        "answer": "40 22",
        "feedback": "var`m` and var`n` are bound to `40`.\nThe variable assignment mutates the binding of var`n`.\nSo, var`n` is eventually bound to `22`.\n",
        "misconceptions": {
          "22 22": {
            "misconception": "DefByRef",
            "feedback": "You might think term`(defvar n m)` binds var`n` to var`m`, so changing var`m` will change var`n`.\nHowever, we learned in previous tutorials that variables are bound to values, so var`n` is bound to the `40`, i.e., *the value of* var`m`.\nIn SMoL, variable assignments change *only* the mutated variables.\n"
          }
        },
        "again": {
          "program": "(defvar zzz 1)\n(defvar abc zzz)\n(set! abc 2)\nzzz\nabc\n",
          "answer": "1 2"
        }
      },
      "not_aliased_by_funarg_2": {
        "program": "(defvar x 12)\n(deffun (f y)\n  (set! y 0)\n  x)\n(f x)\n",
        "answer": "12",
        "feedback": "The function call binds var`y` to `12`.\nThe variable assignment mutates the value of var`y` to `0`, but var`x` is still bound to `12`.\n",
        "misconceptions": {
          "0": {
            "misconception": "CallByRef",
            "feedback": "You might think the function call term`(f x)` binds var`y` to var`x`, so\nchanging var`y` will change var`x`.\nHowever, we learned in previous tutorials that variables are bound to values, so var`y` is bound to `12`, i.e., *the value of* var`x`.\nIn SMoL, variable assignments change *only* the mutated variables.\n"
          },
          "error": {
            "misconception": "IsolatedFun",
            "feedback": "You might think var`f` can't refer to var`x`.\nHowever, var`f` is defined in the top-level block,\nso its body can refer to the top-level var`x`.\nIn SMoL, variable references follow the hierarchical structure of blocks.\n"
          }
        },
        "again": {
          "program": "(defvar s 1)\n(deffun (foobar t)\n  (set! t 2)\n  s)\n(foobar s)\n",
          "answer": "1"
        }
      },
      "new_not_aliased_by_funarg": {
        "program": "(defvar x 12)\n(deffun (f y)\n  (set! x 0)\n  y)\n(f x)\n",
        "answer": "12",
        "feedback": "The function call binds var`y` to `12`.\nThe variable assignment mutates the value of var`x` to `0`, but var`y` is still bound to `12`.\n",
        "misconceptions": {
          "0": {
            "misconception": "CallByRef",
            "feedback": "You might think the function call term`(f x)` binds var`y` to var`x`, so\nchanging var`x` will change var`y`.\nHowever, we learned in previous tutorials that variables are bound to values, so var`y` is bound to `12`, i.e., *the value of* var`x`.\nIn SMoL, variable assignments change *only* the mutated variables.\n"
          },
          "error": {
            "misconception": "IsolatedFun",
            "feedback": "You might think term`(set! x 0)` can't refer to var`x`.\nHowever, term`(set! x 0)` appears in the body of var`g`, which is a sub-block of the top-level block,\nso it can mutate the top-level var`x`.\nIn SMoL, variable references follow the hierarchical structure of blocks.\n"
          }
        },
        "again": {
          "program": "(defvar a 1)\n(deffun (foobar b)\n  (set! a 2)\n  b)\n(foobar a)\na\n",
          "answer": "1 2"
        }
      },
      "alias_var_in_mvec": {
        "program": "(defvar x 59)\n(defvar v (mvec 68 57 x))\n(set! x 74)\nv\n",
        "answer": "#(68 57 59)",
        "feedback": "var`v` is bound to the vector created by term`(mvec 68 57 x)`.\nterm`(mvec 68 57 x)` is a function call.\nvar`x` is evaluated at the point of evaluating term`(mvec 68 57 x)`.\nThat is why the vector's content is `68`, `57`, and `59`.\nThe subsequent variable assignment mutates var`x`.\nBut this doesn't impact the vector\nbecause the vector refers the value `59` rather than var`x`.\n",
        "misconceptions": {
          "#(68 57 74)": {
            "misconception": "StructByRef",
            "feedback": "You might think the `2`-th and last element of the vector refers to var`x`.\nHowever, vectors refer to values, so it actually refers to `59`, i.e., *the value of* var`x`.\nIn SMoL, variable assignments change *only* the mutated variables.\n"
          }
        },
        "again": {
          "program": "(defvar x 66)\n(defvar v (mvec 82 x 93))\n(set! x 1)\nv\n",
          "answer": "#(82 66 93)"
        }
      },
      "new_alias_mvec_in_mvec_trick": {
        "program": "(defvar x (mvec 55))\n(defvar v (mvec x 55 55))\n(set! x (mvec 66))\nv\n",
        "answer": "#(#(55) 55 55)",
        "feedback": "var`x` is first bound to a one-element vector.\nvar`v` is bound to a three-element vector, of which the first element is the one-element vector.\nterm`(set! x (mvec 66))` binds var`x` to a new vector.\nThis doesn't impact var`v` because\nthe `0`-th element of var`v` is still the one-element vector.\n",
        "misconceptions": {
          "#(#(66) 55 55)": {
            "misconception": "StructByRef",
            "feedback": "You might think the `0`-th and first element of var`v` refers to var`x`.\nHowever, vectors refer to values, so it actually refers to the one-element vector, i.e., *the value of* var`x`.\nIn SMoL, variable assignments change *only* the mutated variables.\n"
          }
        },
        "again": {
          "program": "(defvar x (mvec 0))\n(defvar v (mvec 2 x 3))\n(set! x (mvec 1))\nv\n",
          "answer": "#(2 #(0) 3)"
        }
      },
      "reflect_mutvars_1": {
        "prompt": "What did you learn about variable assignment from these programs?\n"
      },
      "goal_mutvars_1": {
        "emphasize": "Variable assignments change *only* the mutated variables. That is, variables are not aliased.\n\n(Note: some programming languages (e.g., C++ and Rust) allow variables to be aliased.\nHowever, even in those languages, variables are not aliased by default.)\n"
      },
      "goal_mutvars_1_labelling": {
        "select_and_comment": "Please scroll back and select 1-3 programs that make the above point.\n\nYou don't need to select *all* such programs.\n"
      }
    }
  },
  "mutvars2": {
    "misconceptions": [
      "DeepClosure",
      "DefOrSet",
      "IsolatedFun"
    ],
    "order": [
      "intro_more_on_mutvars",
      [
        "update_undefined",
        "new_fun_do_not_rem_val",
        "new_fun_rem_env"
      ],
      "reflect_mutvars_2",
      "goal_mutvars_2",
      "goal_mutvars_2_labelling_1",
      "goal_mutvars_2_labelling_2",
      "reflect_env",
      "goal_env",
      "attention_check1",
      "attention_check2",
      "keyframe_mutable_variables_1",
      "keyframe_mutable_variables_2"
    ],
    "questions": {
      "intro_more_on_mutvars": {
        "confirm": "In this tutorial, we will learn more about **variable assignments** and **mutable variables**.\n"
      },
      "update_undefined": {
        "program": "(set! foobar 2)\nfoobar\n",
        "answer": "error",
        "feedback": "Mutating an undefined variable (in this case, var`foobar`) errors rather than defining the variable.\nSo, this program errors.\n",
        "misconceptions": {
          "2": {
            "misconception": "DefOrSet",
            "feedback": "You might think term`(set! foobar 2)` defines var`foobar`.\nHowever, it errors.\nIn SMoL, variable assignments mutate existing bindings and never create new bindings.\n"
          }
        },
        "again": {
          "program": "(set! foo 42)\nfoo\n",
          "answer": "error"
        }
      },
      "new_fun_do_not_rem_val": {
        "program": "(defvar x 1)\n(deffun (f n)\n  (+ x n))\n(set! x 2)\n(f 30)\n",
        "answer": "32",
        "feedback": "The program binds var`x` to `1` and then defines a function var`f`.\nvar`x` is then bound to `2`. So, term`(f 30)` is term`(+ x 30)`, which is `32`.s\n",
        "misconceptions": {
          "31": {
            "misconception": "DeepClosure",
            "feedback": "You are right that var`x` is bound to `1` when var`f` is bound to a function.\nYou might think the function remembers the value `1`.\nHowever, var`f` does not remember the value of var`x`. Rather, it always\nrefers to the latest value of var`x`.\nIn SMoL, functions refer to the latest values of variables defined outside their definitions.\n"
          },
          "error": {
            "misconception": "IsolatedFun",
            "feedback": "You might think term`(+ x n)` can't refer to var`x`.\nHowever, term`(+ x n)` appears in the body of var`f`, which is a sub-block of\nthe top-level block. So, it can refer to the var`x` defined\nin the top-level block.\nIn SMoL, variable references follow the hierarchical structure of blocks.\n"
          }
        },
        "again": {
          "program": "(defvar o 1)\n(deffun (u t)\n  (+ o t))\n(set! o 9)\n(u 5)\n",
          "answer": "14"
        }
      },
      "new_fun_rem_env": {
        "program": "(defvar x 12)\n(deffun (f)\n  x)\n(deffun (g)\n  (set! x 0)\n  (f))\n(g)\n(set! x 1)\n(f)\n",
        "answer": "0 1",
        "feedback": "There is exactly one variable var`x`.\nThe var`x` in term`(set! x 0)` refers to that variable.\nCalling var`f` evaluates term`(set! x 0)`, which mutates var`x`.\nWhen the value of var`x` is eventually printed, we see the new value.\n",
        "misconceptions": {
          "12 1": {
            "misconception": "DefOrSet",
            "feedback": "You might think term`(set! x 0)` defines var`x`.\nHowever, it will mutate the variable defined outside the functions.\nIn SMoL, variable assignments mutate existing bindings and never create new bindings.\n"
          },
          "12 12": {
            "misconception": "DeepClosure",
            "feedback": "You are right that var`x` is bound to `12` when var`f` is bound to a function.\nYou might think the function remembers the value `12`.\nHowever, var`f` does not remember the value of var`x`. Rather, it always\nrefers to the latest value of var`x`.\nIn SMoL, functions refer to the latest values of variables defined outside their definitions.\n"
          },
          "error": {
            "misconception": "IsolatedFun",
            "feedback": "You might think var`f` can't refer to var`x`.\nHowever, var`f` is defined in the top-level block.\nSo, it can refer to the var`x` defined in the top-level block.\n\nYou might think term`(set! x 0)` can't refer to var`x`.\nHowever, term`(set! x 0)` appears in the body of var`g`, which is a sub-block of\nthe top-level block. So, it can refer to the var`x` defined\nin the top-level block.\n\nIn SMoL, variable references follow the hierarchical structure of blocks.\n"
          }
        },
        "again": {
          "program": "(defvar a 4)\n(deffun (h)\n  a)\n(deffun (k)\n  (set! a 2)\n  (h))\n(k)\n(set! a 6)\n(h)\n",
          "answer": "2 6"
        }
      },
      "reflect_mutvars_2": {
        "prompt": "What did you learn about variable assignment from these programs?\n"
      },
      "goal_mutvars_2": {
        "emphasize": "- Variable assignments mutate existing bindings and do not create new bindings.\n- Functions refer to the latest values of variables defined outside their definitions. That is, functions do not remember the values of those variables from when the functions were defined.\n"
      },
      "goal_mutvars_2_labelling_1": {
        "select_and_comment": "Please scroll back and select 1-3 programs that make the following point:\n\n> Variable assignments mutate existing bindings and do not create new bindings.\n\nYou don't need to select *all* such programs.\n"
      },
      "goal_mutvars_2_labelling_2": {
        "select_and_comment": "Please scroll back and select 1-3 programs that make the following point:\n\n> Functions refer to the latest values of variables defined outside their definitions. That is, functions do not remember the values of those variables from when the functions were defined.\n\nYou don't need to select *all* such programs.\n"
      },
      "reflect_env": {
        "prompt": "Although we keep saying \"this variable is mutated\", the variables themselves are *not* mutated.\nWhat is actually mutated is *the binding between the variables and their values*.\n\nBlocks have been a good way to describe these bindings.\nHowever, they can't explain variable mutations: a block is a piece of source code,\nand we can't change the source code by mutating a variable. Besides,\nblocks can't explain how parameters might be bound differently in different function calls.\nConsider the following program:\n\n```\n(deffun (f n)\n  (+ n 1))\n(f 2)\n(f 3)\n```\n\nIn this program, var`n` is bound to `2` in this first function call, and `3` in the second.\nBlocks can't explain how var`n` is bound differently because the two calls share the same\nblock: the body of var`f`.\n\nWhat is a better way to describe the binding between variables and their values?\n"
      },
      "goal_env": {
        "emphasize": "**Environments** (rather than blocks) bind variables to values.\n\nSimilar to vectors, environments are created as programs run.\n\nEnvironments are created from blocks.\nThey form a tree-like structure,\nrespecting the tree-like structure of their corresponding blocks.\nSo, we have a primordial environment, a top-level environment,\nand environments created from function bodies.\n\n*Every function call creates a new environment.* This is very different from\nthe block perspective: every function corresponds to exactly one block, its body.\n"
      },
      "attention_check1": {
        "prompt": "Which language construct(s) create new bindings?",
        "checkboxes": [
          [
            true,
            "Definitions"
          ],
          [
            false,
            "Variable mutations (e.g., term`(set! x 3)`)"
          ],
          [
            false,
            "Variable references"
          ],
          [
            true,
            "Function calls"
          ]
        ],
        "feedback": "Both definitions and function calls create bindings.\nVariable assignment mutates existing bindings but doesn't create new bindings.\n"
      },
      "attention_check2": {
        "prompt": "Which language construct(s) mutate bindings?",
        "checkboxes": [
          [
            false,
            "Definitions"
          ],
          [
            true,
            "Variable mutations (e.g., term`(set! x 3)`)"
          ],
          [
            false,
            "Variable references"
          ],
          [
            false,
            "Function calls"
          ]
        ],
        "feedback": "Only variable assignments mutate bindings.\nDefinitions and function calls create new bindings but don't mutate existing bindings.\n"
      },
      "keyframe_mutable_variables_1": {
        "prompt": "Here is a program that confused many students\n\n```\n(defvar x 5)\n(deffun (f x y)\n  (set! x y))\n(f x 6)\nx\n```\n\nPlease\n\n1. Run this program in the stacker by clicking the green run button above;\n2. The stacker would show how this program produces its result(s);\n3. Keep clicking <button disabled>⏭ Next</button> until you reach a configuration that you find particularly helpful;\n4. Click <button disabled>🔗 Share This Configuration</button> to get a link to your configuration;\n5. Submit your link below;\n"
      },
      "keyframe_mutable_variables_2": {
        "prompt": "Please write a couple of sentences to explain how your configuration explains the result(s) of the program.\n"
      }
    }
  },
  "lambda1": {
    "misconceptions": [
      "FunNotVal"
    ],
    "order": [
      "intro_lambda",
      [
        "fun_as_val",
        "fun_as_parameter",
        "new_fun_as_output",
        "fun_in_vectors"
      ],
      "reflect_funval",
      "goal_funval",
      "goal_funval_labelling"
    ],
    "questions": {
      "intro_lambda": {
        "confirm": "In this tutorial, we will learn about using functions as values.\n\nNote: some programming languages do NOT consider functions or methods a kind of value.\nBut many programming languages, from Python to Rust, do.\n"
      },
      "fun_as_val": {
        "program": "(deffun (f)\n  42)\n(defvar g f)\n(defvar h g)\n(h)\n",
        "answer": "42",
        "feedback": "This program binds var`f` to a function that returns `42`,\nand then binds var`g` and var`h` to that function.\nFinally, calling that function produces `42`.\n",
        "misconceptions": {
          "error": {
            "misconception": "FunNotVal",
            "feedback": "You might think that term`(defvar g f)` errors because var`f` is bound to a function,\nand that it would not error if var`f` is bound to a value of other kinds (e.g., numbers and vectors).\nHowever, it does not error.\nIn SMoL, functions are (also) *first-class* citizens of the value world.\n"
          }
        },
        "again": {
          "program": "(deffun (f1)\n  5)\n(defvar f2 f1)\n(defvar f3 f2)\n(* (f3) 10)\n",
          "answer": "50"
        }
      },
      "fun_as_parameter": {
        "program": "(deffun (twice f x)\n  (f (f x)))\n(deffun (double x)\n  (+ x x))\n(twice double 1)\n",
        "answer": "4",
        "feedback": "The value of term`(twice double 1)` is the value of term`(f (f x))`, where\nvar`f` is bound to var`double` and var`x` is bound to `1`.\nSo, the result is the value of term`(double (double 1))`, which is `4`.\n",
        "misconceptions": {
          "error": {
            "misconception": "FunNotVal",
            "feedback": "You might think that term`(twice double 1)` errors because var`double` is bound to a function,\nand that it would not error if var`double` is bound to a value of other kinds (e.g., numbers and vectors).\nHowever, it does not error.\nIn SMoL, functions are (also) *first-class* citizens of the value world.\n"
          }
        },
        "again": {
          "program": "(deffun (ffx f x)\n  (f (f x)))\n(deffun (inc x)\n  (+ x 1))\n(ffx inc 1)\n",
          "answer": "3"
        }
      },
      "new_fun_as_output": {
        "program": "(deffun (inc x)\n  (+ x 1))\n(deffun (g)\n  inc)\n(defvar f (g))\n(f 10)\n",
        "answer": "11",
        "feedback": "var`inc` is bound to a function that adds 1 to its parameter.\nvar`g` is bound to a function that returns var`inc`.\nvar`f` is bound to the value of term`(g)`, which is var`inc`.\nSo, the value of term`(f 10)` is `11`.\n",
        "misconceptions": {
          "error": {
            "misconception": "FunNotVal,IsolatedFun",
            "feedback": "You might think that term`(g)` errors because it returns a function,\nand that it would not error if it returns a value of other kinds (e.g., numbers and vectors).\nHowever, it does not error.\nIn SMoL, functions are (also) *first-class* citizens of the value world.\n"
          }
        },
        "again": {
          "program": "(deffun (fun1)\n  (deffun (average x y)\n    (/ (+ x y) 2))\n  average)\n(defvar x (fun1))\n(x 20 40)\n",
          "answer": "30"
        }
      },
      "fun_in_vectors": {
        "program": "(deffun (inc n)\n  (+ n 1))\n(defvar v (mvec inc inc))\n((vec-ref v 0) 2)\n",
        "answer": "3",
        "feedback": "var`v` is bound to a vector that refers to the function var`inc`.\nThe value of term`(vec-ref v 0)` is the function var`inc`.\nSo, the value of term`((vec-ref v 0) 2)` is the value of term`(inc 2)`,\nwhich is `3`.\n",
        "misconceptions": {
          "error": {
            "misconception": "FunNotVal",
            "feedback": "You might think that term`(mvec inc)` errors because var`inc` is bound to a function,\nand that it would not error if var`inc` is bound to a value of other kinds (e.g., numbers and vectors).\nHowever, it does not error.\nIn SMoL, functions are (also) *first-class* citizens of the value world.\n"
          }
        },
        "again": {
          "program": "(deffun (inc n)\n  (+ n 1))\n(deffun (dec n)\n  (- n 1))\n(defvar v (mvec inc dec))\n((vec-ref v 1) 2)\n",
          "answer": "1"
        }
      },
      "reflect_funval": {
        "prompt": "What did you learn about functions from these programs?\n"
      },
      "goal_funval": {
        "emphasize": "Functions are (also) *first-class* citizens of the value world. Specifically,\n\n- Variables (notably parameters) can be bound to functions,\n- Functions can return functions, and\n- Vectors can refer to functions.\n"
      },
      "goal_funval_labelling": {
        "select_and_comment": "Please scroll back and select 1-3 programs that make the point above.\n\nYou don't need to select *all* such programs.\n"
      }
    }
  },
  "lambda2": {
    "misconceptions": [
      "FlatEnv",
      "DeepClosure"
    ],
    "order": [
      "intro_more_funval",
      [
        "fun_ref_env",
        "new_state",
        "counter"
      ],
      "reflect_closure",
      "goal_closure",
      "label_closure"
    ],
    "questions": {
      "intro_more_funval": {
        "confirm": "In this tutorial, we will learn *more* about using functions as values.\n"
      },
      "fun_ref_env": {
        "program": "(deffun (bar y)\n  (deffun (addy x)\n    (+ x y))\n  addy)\n(defvar f (bar 2))\n(defvar g (bar 4))\n(f 2)\n(g 2)\n",
        "answer": "4 6",
        "feedback": "The value of term`(bar 2)` is the function var`addy` defined in an\nenvironment where var`y` is bound to `2`.\nThe value of term`(bar 4)` is *another* var`addy` defined in an\nenvironment where var`y` is bound to `4`.\nThe two var`addy` functions are *different* values.\nSo, the value of term`(f 2)` is `4`,\nwhile the value of term`(g 2)` is `6`.\n",
        "misconceptions": {
          "6 6": {
            "misconception": "FlatEnv",
            "feedback": "var`bar` is called twice.\nThe first call binds var`y` to `2`.\nThe second call binds var`y` to `4`.\nYou might think the second binding applies everywhere.\nHowever, it only applies to the second call.\nThe var`addy` that var`f` refers to is created in the first call, so term`(f 2)` adds `2` rather than `4`.\nIn SMoL, variable references follow the hierarchical structure of blocks.\n"
          },
          "error": {
            "misconception": "FunNotVal,IsolatedFun",
            "feedback": "You might think that term`(bar 2)` and term`(bar 4)` errors because they return functions,\nand that they would not error if they return values of other kinds (e.g., numbers and vectors).\nHowever, they do not error.\n\nYou might think that the two variable definitions error because the variables are bound to functions,\nand that they would not error if the variables were bound to values of other kinds (e.g., numbers and vectors).\nHowever, they do not error.\n\nIn SMoL, functions are (also) *first-class* citizens of the value world.\n"
          }
        },
        "again": {
          "program": "(deffun (f n)\n  (deffun (g m)\n    (* m n))\n  g)\n(defvar fun1 (f 10))\n(defvar fun2 (f 1))\n(fun1 4)\n(fun2 4)\n",
          "answer": "40 4"
        }
      },
      "new_state": {
        "program": "(defvar x 1)\n(deffun (f)\n  (deffun (addx y)\n    (+ x y))\n  addx)\n(defvar g (f))\n(set! x 2)\n(g 0)\n",
        "answer": "2",
        "feedback": "var`x` is bound to `1`.\nvar`g` is bound to the function var`addx`\nterm`(set! x 2)` binds var`x` to `2`.\nSo, the value of term`(g 0)` is the value of term`(addx 0)`, which is the value of term`(+ 2 0)`, which is `2`.\n",
        "misconceptions": {
          "1": {
            "misconception": "DeepClosure",
            "feedback": "You are right that var`x` is bound to `1` when var`addx` is created.\nYou might think the function remembers the value `1`.\nHowever, var`addx` does not remember the value of var`x`.  Rather, it remembers the environment and hence always refers to the latest value of var`n`.\nIn SMoL, functions refer to the latest values of variables defined outside their definitions.\n"
          },
          "error": {
            "misconception": "FunNotVal,IsolatedFun",
            "feedback": "You might think that term`(f)` errors because it returns a function,\nand that it would not error if it returns a value of other kinds (e.g., numbers and vectors).\nHowever, it does not error.\n\nYou might think that term`(defvar g (f))` errors because var`g` is bound to a function,\nand that it would not error if var`g` is bound to a value of other kinds (e.g., numbers and vectors).\nHowever, it does not error.\n\nIn SMoL, functions are (also) *first-class* citizens of the value world.\n"
          }
        },
        "again": {
          "program": "(defvar a 2)\n(deffun (make)\n  (deffun (f b)\n    (+ a b))\n  f)\n(defvar g (make))\n(set! a 1)\n(g 1)\n",
          "answer": "2"
        }
      },
      "counter": {
        "program": "(deffun (foo)\n  (defvar n 0)\n  (deffun (bar)\n    (set! n (+ n 1))\n    n)\n  bar)\n(defvar f (foo))\n(defvar g (foo))\n\n(f)\n(f)\n(g)\n",
        "answer": "1 2 1",
        "feedback": "Every time var`foo` is called, it creates a *new* environment frame that binds var`n`.\nSo, var`f` and var`g` have different bindings for the var`n` variable.\nWhen var`f` is called the first time, it mutates its binding for the var`n` variable.\nSo, the second call to var`f` produces `2` rather than `1`.\nvar`g` has its own binding for the var`n` variable, which still binds var`n` to `0`.\nSo, term`(g)` produces `1` rather than `3`.\n",
        "misconceptions": {
          "1 2 3": {
            "misconception": "FlatEnv",
            "feedback": "var`foo` is called twice.\nThe first call binds var`n` to `0`.\nThe second call also binds var`n` to `0`.\nYou might think the second binding applies everywhere.\nHowever, it only applies to the second call.\nterm`(f)` mutates the first binding, while term`(g)` mutates the second.\nIn SMoL, variable references follow the hierarchical structure of blocks.\n"
          },
          "1 1 1": {
            "misconception": "DeepClosure",
            "feedback": "You are right that var`n` is bound to `0` when var`bar` is bound to a function.\nYou might think the function remembers the value `0`.\nHowever, var`bar` does not remember the value of var`n`. Rather, it remembers the environment and hence always refers to the latest value of var`n`.\nvar`foo` is called twice, so two environments are created.\nterm`(f)` mutates the first, while term`(g)` mutates the second.\nIn SMoL, functions refer to the latest values of variables defined outside their definitions.\n"
          },
          "error": {
            "misconception": "DefOrSet,FunNotVal,IsolatedFun"
          }
        },
        "again": {
          "program": "(deffun (f)\n  (defvar n 1)\n  (deffun (dbl)\n    (set! n (* n 2))\n    n)\n  dbl)\n(defvar dbl1 (f))\n(defvar dbl2 (f))\n\n(dbl1)\n(dbl2)\n(dbl1)\n",
          "answer": "2 2 4"
        }
      },
      "reflect_closure": {
        "prompt": "What did you learn about functions from these programs?"
      },
      "goal_closure": {
        "emphasize": "Functions remember the environment in which they are defined.\nThat is, function bodies are \"enclosed\" by the environments in which the function values are created.\nSo, function values are called *closures*.\n"
      },
      "label_closure": {
        "select_and_comment": "Please scroll back and select 1-3 programs that make the point that\n\n> Functions remember the environment in which they are defined.\n\nYou don't need to select *all* such programs.\n"
      }
    }
  },
  "lambda3": {
    "misconceptions": [
      "FlatEnv",
      "DeepClosure"
    ],
    "order": [
      "intro_lambda",
      "warmup_lambda",
      [
        "lambda_fun_ref_env",
        "lambda_new_state",
        "lambda_counter"
      ],
      "goal_lambda",
      "practice_translate",
      "keyframe_counter_1",
      "keyframe_counter_2"
    ],
    "questions": {
      "intro_lambda": {
        "emphasize": "In this tutorial, we will learn about **lambda expressions**, which are expressions that create functions.\n\nThe following program illustrates how to create a function.\n\n```term\n((lambda (n)\n   (+ n 1))\n 2)\n```\n\nThis program program produces `3`. The top-level block contains one expression, a function call. The only (actual) parameter of the function call is `2`. The function of the function call is created by\n\n```term\n(lambda (n)\n  (+ n 1))\n```\n\nThis function takes only one parameter var`n`, and returns (the value of) term`(+ n 1)`.\nSo, the result of the whole program is the value of term`(+ 2 1)`, which is `3`.\n"
      },
      "warmup_lambda": {
        "program": "(deffun (f x)\n  (lambda (y) (+ x y)))\n(defvar x 0)\n((f 2) 1)\n",
        "answer": "3",
        "feedback": "This program is essentially the same as\n\n```\n(deffun (f x)\n  (deffun (fun y)\n    (+ x y))\n  fun)\n(defvar x 0)\n((f 2) 1)\n```\n",
        "again": {
          "program": "(deffun (g a)\n  (lambda (b) (+ a b)))\n\n((g 3) 2)\n",
          "answer": "5"
        }
      },
      "lambda_fun_ref_env": {
        "program": "(deffun (bar y)\n  (lambda (x)\n    (+ x y)))\n(defvar f (bar 2))\n(defvar g (bar 4))\n(f 2)\n(g 2)\n",
        "answer": "4 6",
        "feedback": "The value of term`(bar 2)` is a lambda function defined in an\nenvironment where var`y` is bound to `2`.\nThe value of term`(bar 4)` is *another* lambda function defined in an\nenvironment where var`y` is bound to `4`.\nThe two lambda functions are *different* values.\nSo, the value of term`(f 2)` is `12`,\nwhile the value of term`(g 2)` is `52`.\n",
        "misconceptions": {
          "6 6": {
            "misconception": "FlatEnv",
            "feedback": "var`bar` is called twice.\nThe first call binds var`y` to `2`.\nThe second call binds var`y` to `4`.\nYou might think the second binding applies everywhere.\nHowever, it only applies to the second call.\nThe lambda function that var`f` refers to is created in the first call, so term`(f 2)` adds `2` rather than `4`.\nIn SMoL, variable references follow the hierarchical structure of blocks.\n"
          },
          "error": {
            "misconception": "FunNotVal,IsolatedFun",
            "feedback": "You might think that term`(bar 2)` and term`(bar 4)` errors because they return functions,\nand that they would not error if they return values of other kinds (e.g., numbers and vectors).\nHowever, they do not error.\n\nYou might think that the two variable definitions error because the variables are bound to functions,\nand that they would not error if the variables were bound to values of other kinds (e.g., numbers and vectors).\nHowever, they do not error.\n\nIn SMoL, functions are (also) *first-class* citizens of the value world.\n"
          }
        },
        "again": {
          "program": "(deffun (f n)\n  (lambda (m)\n    (* m n)))\n(defvar fun1 (f 10))\n(defvar fun2 (f 1))\n(fun1 4)\n(fun2 4)\n",
          "answer": "40 4"
        }
      },
      "lambda_new_state": {
        "program": "(defvar x 1)\n(deffun (f)\n  (lambda (y)\n    (+ x y)))\n(defvar g (f))\n(set! x 2)\n(g 0)\n",
        "answer": "2",
        "feedback": "var`x` is bound to `1`.\nvar`g` is bound to the lambda function.\nterm`(set! x 2)` binds var`x` to `2`.\nSo, the value of term`(g 0)` is the value of term`(+ 2 0)`, which is `2`.\n",
        "misconceptions": {
          "1": {
            "misconception": "DeepClosure",
            "feedback": "You are right that var`x` is bound to `1` when the lambda function is created.\nYou might think the function remembers the value `1`.\nHowever, the function does not remember the value of var`x`.  Rather, it remembers the environment and hence always refers to the latest value of var`x`.\nIn SMoL, functions refer to the latest values of variables defined outside their definitions.\n"
          },
          "error": {
            "misconception": "FunNotVal,IsolatedFun",
            "feedback": "You might think that term`(f)` errors because it returns a function,\nand that it would not error if it returns a value of other kinds (e.g., numbers and vectors).\nHowever, it does not error.\n\nYou might think that term`(defvar g (f))` errors because var`g` is bound to a function,\nand that it would not error if var`g` is bound to a value of other kinds (e.g., numbers and vectors).\nHowever, it does not error.\n\nIn SMoL, functions are (also) *first-class* citizens of the value world.\n"
          }
        },
        "again": {
          "program": "(defvar a 2)\n(deffun (make)\n  (lambda (b)\n    (+ a b)))\n(defvar fun (make))\n(set! a 1)\n(fun 1)\n",
          "answer": "2"
        }
      },
      "lambda_counter": {
        "program": "(deffun (foobar)\n  (defvar n 0)\n  (lambda ()\n    (set! n (+ n 1))\n    n))\n(defvar f (foobar))\n(defvar g (foobar))\n\n(f)\n(f)\n(g)\n",
        "answer": "1 2 1",
        "feedback": "Every time var`foobar` is called, it creates a *new* environment that binds var`n`.\nSo, var`f` and var`g` have different bindings for the var`n` variable.\nWhen var`f` is called the first time, it mutates its binding for the var`n` variable.\nSo, the second call to var`f` produces `2` rather than `1`.\nvar`g` has its own binding for the var`n` variable, which still binds var`n` to `0`.\nSo, term`(g)` produces `1` rather than `3`.\n",
        "misconceptions": {
          "1 2 3": {
            "misconception": "FlatEnv",
            "feedback": "var`foobar` is called twice.\nThe first call binds var`n` to `0`.\nThe second call also binds var`n` to `0`.\nYou might think the second binding applies everywhere.\nHowever, it only applies to the second call.\nterm`(f)` mutates the first binding, while term`(g)` mutates the second.\nIn SMoL, variable references follow the hierarchical structure of blocks.\n"
          },
          "1 1 1": {
            "misconception": "DeepClosure",
            "feedback": "You are right that var`n` is bound to `0` when the lambda function is created.\nYou might think the function remembers the value `0`.\nHowever, the lambda function does not remember the value of var`n`. Rather, it remembers the environment and hence always refers to the latest value of var`n`.\nvar`foobar` is called twice, so its parameter, var`n`, is bound twice.\nterm`(f)` mutates the first binding, while term`(g)` mutates the second.\nIn SMoL, functions refer to the latest values of variables defined outside their definitions.\n"
          },
          "error": {
            "misconception": "DefOrSet,FunNotVal,IsolatedFun"
          }
        },
        "again": {
          "program": "(deffun (build-dbl)\n  (defvar n 1)\n  (lambda ()\n    (set! n (* n 2))\n    n))\n(defvar dbl1 (build-dbl))\n(defvar dbl2 (build-dbl))\n\n(dbl1)\n(dbl2)\n(dbl1)\n",
          "answer": "2 2 4"
        }
      },
      "goal_lambda": {
        "emphasize": "term`(deffun (f x y z) body)` is a shorthand for term`(defvar f (lambda (x y z) body))`.\n"
      },
      "practice_translate": {
        "prompt": "Please rewrite this function definition with as a variable definition that binds lambda function.\n\n```\n(deffun (f x) (+ x 1))\n```\n",
        "feedback": "The correct answer is term`(defvar f (lambda (x) (+ x 1)))`.\n\nThe following are common wrong answers:\n\n- term`(deffun f (lambda (x) (+ x 1)))`, which didn't replace the definition keyword\n- term`(defvar f (lambda (f x) (+ x 1)))`, which makes var`f` a function that takes two parameters rather than one\n"
      },
      "keyframe_counter_1": {
        "prompt": "Here is a program that confused many students\n\n```\n(deffun (foo n)\n  (deffun (bar)\n    (set! n (+ n 1))\n    n)\n  bar)\n(defvar f (foo 0))\n(defvar g (foo 0))\n\n(f)\n(f)\n(g)\n```\n\nPlease\n\n1. Run this program in the stacker by clicking the green run button above;\n2. The stacker would show how this program produces its result(s);\n3. Keep clicking <button disabled>⏭ Next</button> until you reach a configuration that you find particularly helpful;\n4. Click <button disabled>🔗 Share This Configuration</button> to get a link to your configuration;\n5. Submit your link below;\n"
      },
      "keyframe_counter_2": {
        "prompt": "Please write a couple of sentences to explain how your configuration explains the result(s) of the program.\n"
      }
    }
  },
  "begin": {
    "misconceptions": [
      "Other"
    ],
    "order": [
      "intro_begin",
      [
        "warmup_begin_1",
        "warmup_begin_2",
        "begin_setbang",
        "begin_can_take_three",
        "intro_if",
        "if_begin_1",
        "if_begin_3",
        "if_begin_4"
      ],
      "reflect_begin",
      "goal_begin"
    ],
    "questions": {
      "intro_begin": {
        "confirm": "In this tutorial, we will learn about `begin`.\n\n`begin` lets you evaluate a sequence of expressions and returns the value of the last expression.\n"
      },
      "warmup_begin_1": {
        "program": "(defvar x (begin 2 3 4))\nx\n",
        "answer": "4",
        "feedback": "The definition binds var`x`  to the value of the `begin` expression.\nA `begin` expression evaluates its sub-expressions from left to right\nand produces the value of the right-most expression.\nSo, var`x` is bound to `4`.\n",
        "misconceptions": {
          "2 3 4": {
            "misconception": "Other"
          }
        },
        "again": {
          "program": "(defvar bval (begin 7 6 5))\nbval\n",
          "answer": "5"
        }
      },
      "warmup_begin_2": {
        "program": "(defvar z\n  (begin\n    (begin\n      6\n      45)\n    (begin\n      67)))\nz\n",
        "answer": "67",
        "feedback": "The value of term`(begin (begin 6 45) (begin 67))` is the value of\nterm`(begin 67)`, which is `67`.\n",
        "again": {
          "program": "(defvar res\n  (begin\n    (begin\n      67)\n    (begin\n      (begin 5)\n      45)))\nres\n",
          "answer": "45"
        }
      },
      "begin_setbang": {
        "program": "(defvar x 0)\n(defvar y\n  (begin\n    (set! x (+ x 1))\n    x))\nx\ny\n",
        "answer": "1 1",
        "feedback": "The `begin` first increases var`x` by one, then evaluates var`x`. So, the value of var`y` is `1`.",
        "misconceptions": {
          "1 0": {
            "misconception": "Other"
          },
          "0 1": {
            "misconception": "Other"
          },
          "0 0": {
            "misconception": "Other"
          }
        },
        "again": {
          "program": "(defvar a 30)\n(defvar b\n  (begin\n    (set! a (* a 2))\n    a))\na\nb\n",
          "answer": "60 60"
        }
      },
      "begin_can_take_three": {
        "program": "(defvar x 0)\n(defvar y x)\n(defvar z\n  (begin\n    (set! x (+ x 1))\n    (set! y 4)\n    x))\nx\ny\nz\n",
        "answer": "1 4 1",
        "feedback": "At first, var`x` is bound to `0`, and var`y` is bound to `0`.\nThe `begin` mutates var`x` to `1` and var`y` to `4`,\nand returns the value of var`x`, which is now `1`.\n",
        "again": {
          "program": "(defvar a 2)\n(defvar b a)\n(defvar c\n  (begin\n    (set! a (* a 3))\n    (set! b 32)\n    a))\na\nb\nc\n",
          "answer": "6 32 6"
        }
      },
      "intro_if": {
        "confirm": "The next few questions are going to combine `begin` with var`if`.\n\nterm`(if cnd thn els)` evaluates var`cnd`.\nIf the evaluation produces a true value, var`thn` is evaluated in place of the whole var`if` expression.\nOtherwise, var`els` is evaluated.\n\nGiving var`if` too few or too many subexpressions is a syntactic error.\nFor example, all of the following programs produce errors.\n\n```\n(if (eq? 2 3)\n    \"Do something\")\n```\n\n```\n(if (eq? \"apple\" \"orange\")\n    \"What?!\"\n    \"Okay\"\n    \"...\")\n```\n\nNow let's move on to other questions.\n"
      },
      "if_begin_1": {
        "program": "(defvar counter 0)\n(deffun (factorial n)\n  (if (eq? n 0)\n      1\n      (set! counter (+ counter 1))\n      (* (factorial (- n 1)) n)))\n(factorial 2)\n",
        "answer": "error",
        "feedback": "This is a syntax error!\nThere are 4 sub-expressions in the var`if` expression: term`(eq? n 0)` and `1` and term`(set! ...)` and term`(* ...)`.\nAs a result, the language doesn't know which is the condition, which is the then branch, and which is the else branch.\n",
        "again": {
          "program": "(defvar x 7)\n(deffun (f n)\n  (if (> n 88)\n      1\n      (set! x (+ x 1))\n      (* (f (+ n 1)) n)))\n(f 45)\n",
          "answer": "error"
        }
      },
      "if_begin_3": {
        "program": "(defvar counter 0)\n(deffun (factorial n)\n  (if (eq? n 0)\n      1\n      (begin\n        (set! counter (+ counter 1))\n        (* (factorial (- n 1)) n))))\n(factorial 2)\n",
        "answer": "2",
        "feedback": "Finally, the program is correct!\n",
        "again": {
          "program": "(defvar x 0)\n(deffun (fib n)\n  (if (<= n 1)\n      1\n      (begin\n        (set! x (+ x 1))\n        (+ (fib (- n 1))\n           (fib (- n 2))))))\n(fib 2)\n",
          "answer": "2"
        }
      },
      "if_begin_4": {
        "program": "(defvar counter 0)\n(deffun (factorial n)\n  (if (eq? n 0) 1\n      (begin\n        (set! counter (+ counter 1))\n        (* (factorial (- n 1)) n))))\n(factorial 2)\n",
        "answer": "2",
        "feedback": "This program is still correct! The var`then` branch (`1`) is on the same line as the condition (term`(eq? n 0)`).\n",
        "again": {
          "program": "(defvar x 0)\n(deffun (fib n)\n  (if (<= n 1) 1\n      (begin\n        (set! x (+ x 1))\n        (+ (fib (- n 1))\n           (fib (- n 2))))))\n(fib 2)\n",
          "answer": "2"
        }
      },
      "reflect_begin": {
        "prompt": "What did you learn about `begin` from these programs?"
      },
      "goal_begin": {
        "emphasize": "`begin` computes its sub-expressions in top-to-bottom, left-to-right order, and gives the value of the last expression.\n"
      }
    }
  },
  "local": {
    "misconceptions": [
      "Other"
    ],
    "order": [
      "intro_local",
      "warmup_nested",
      "warmup_local",
      "shadow",
      "goal_bind",
      "syntax_error",
      "all_let",
      "classifier_let_1",
      "classifier_let_2",
      "classifier_let_3",
      "reflect_let",
      "goal_let",
      "intro_letrec",
      "goal_letrec",
      "intro_letstar",
      "goal_letstar",
      "swap_let",
      "swap_letrec",
      "swap_letstar"
    ],
    "questions": {
      "intro_local": {
        "confirm": "In this tutorial, we will learn about *local binding forms*: `let` and `letrec` and `let*`.\n\nWe will first focus on `let`. The following program *binds* var`x` to `2` and var`y` to `3`, and evaluates\nterm`(+ x y)`. The result of running this program is `5`.\n\n```\n(let ([x 2]\n      [y (+ 2 1)])\n  (+ x y))\n```\n\nIn SMoL, `let` binds the *left-hand-side variables*\n(in this case, var`x` and var`y`) to\nthe values of the *right-hand-side expressions* (in this case, `2` and term`(+ 2 1)`)\nsuch that the *body* (in this case, term`(+ x y)`)\ncan refer to all those left-hand-side variables.\n\nFurthermore, the bindings are *local*,\nwhich means we can't refer to the `let`-bound variables outside the `let`.\n\nFor example, the following program produces `4 10`.\n\n```\n(defvar x 10)\n(let ([x 4])\n  x)\nx\n```\n"
      },
      "warmup_nested": {
        "program": "(let ([x 2])\n  (let ([y 30])\n    (+ x y)))\n",
        "answer": "32",
        "feedback": "",
        "again": {
          "program": "(let ([a 10])\n  (let ([b 1])\n    (- a b)))\n",
          "answer": "9"
        }
      },
      "warmup_local": {
        "program": "(defvar x 3)\n(let ([x 5])\n  (+ x 1))\n(+ x 2)\n",
        "answer": "6 5",
        "feedback": "This program includes two blocks: the top-level block and the body of the `let` expression.\nThe first addition (i.e., term`(+ x 1)`) refers to the local var`x`, the var`x` defined in the body of the `let` expression.\nSo, it produces the value of term`(+ 5 1)`, which is `6`.\nIn contrast, the second addition (i.e., term`(+ x 2)`) refers to the top-level var`x`.\nSo, it produces the value of term`(+ 3 2)`, which is `5`.\n",
        "misconceptions": {
          "4 5": {
            "misconception": "Other"
          },
          "4 7": {
            "misconception": "Other"
          },
          "6 7": {
            "misconception": "Other"
          }
        },
        "again": {
          "program": "(defvar vr 7)\n(let ([vr 1])\n  (+ vr 4))\n(+ vr 5)\n",
          "answer": "5 12"
        }
      },
      "shadow": {
        "program": "(defvar x 3)\n(defvar y (let ([x 5]) x))\n(* x y)\n",
        "answer": "15",
        "feedback": "The top-level var`x` is `3`.\nThe top-level var`y` is bound to the value of var`x` in the `let`, which is `5`.\nSo, the product is `15`.\n",
        "misconceptions": {
          "9": {
            "misconception": "Other"
          },
          "25": {
            "misconception": "Other",
            "feedback": "The `let` binds var`x` to `5` in its body.\nBut in the top-level environment, var`x` is still bound to `3`.\n"
          }
        },
        "again": {
          "program": "(defvar a 30)\n(defvar b (let ([a 2]) a))\n(* a b)\n",
          "answer": "60"
        }
      },
      "goal_bind": {
        "emphasize": "A `let` expression binds its left-hand-side variables such that\nits body is in the scope of all the variables.\n\n(Recall that the scope of a variable is the region of a program where the\nvariable can be referred to.)\n"
      },
      "syntax_error": {
        "program": "(let (foo 41)\n  (+ foo 1))\n",
        "answer": "error",
        "feedback": "This program has a syntax error!\nIt should say term`([foo 41])` rather than term`(foo 41)`.\n",
        "misconceptions": {
          "42": {
            "misconception": "Other"
          }
        },
        "again": {
          "program": "(let (x 2)\n  (+ x 1))\n",
          "answer": "error"
        }
      },
      "all_let": {
        "confirm": "What we have seen so far also applies to `letrec` and `let*`.\n"
      },
      "classifier_let_1": {
        "program": "(let ([x 2]\n      [y (+ x 1)])\n   (+ x y))\n",
        "answer": "error",
        "feedback": "The program errors because term`(+ x 1)` is not in the scope of var`x`.\n",
        "misconceptions": {
          "5": {
            "misconception": "Other"
          }
        },
        "again": {
          "program": "(let ([a 2]\n      [b (* a 3)])\n   (+ a b))\n",
          "answer": "error"
        }
      },
      "classifier_let_2": {
        "program": "(let ([x 42]\n      [getx (lambda () x)])\n  (getx))\n",
        "answer": "error",
        "feedback": "The scope of var`x` only includes the `let` body (i.e., term`(getx)`)\nand does NOT include any of the right-hand-side expressions (i.e., `42` and the var`lambda` expression).\nSo, the var`x` in the  var`lambda` expression is undefined.\n",
        "misconceptions": {
          "42": {
            "misconception": "Other"
          }
        },
        "again": {
          "program": "(let ([foo 5]\n      [getter (lambda () foo)])\n  (getter))\n",
          "answer": "error"
        }
      },
      "classifier_let_3": {
        "program": "(let ([f (lambda (n)\n           (or (eq? n 0)\n               (f (- n 1))))])\n  (f 3))\n",
        "answer": "error",
        "feedback": "The program has a scope error.\nThe var`lambda` expression is not in the scope of var`f`.\nSo, it can not refer to itself.\n",
        "misconceptions": {
          "#t": {
            "misconception": "Other"
          }
        },
        "again": {
          "program": "(let ([h (lambda (x)\n           (or (eq? x 64)\n               (h (* x 2))))])\n  (h 2))\n",
          "answer": "error"
        }
      },
      "reflect_let": {
        "prompt": "What is the scope of a left-hand-side variable of `let`?"
      },
      "goal_let": {
        "emphasize": "In a `let`, the scope of a left-hand-side variable includes *none* of the right-hand-side expressions.\n\nIf we put it visually,\n\n<image src=\"data:image/png;base64,\"/>\n"
      },
      "intro_letrec": {
        "confirm": "`letrec` and `let*` are similar to `let`: all of them declare their left-hand-side variables such that\ntheir bodies are in the scope of all the left-hand side variables.\n\nThe following program tries to use `let` to define the factorial function.\n\n```\n(let ([f (lambda (n)\n           (if (eq? n 0)\n               1\n               (* (f (- n 1)) n)))])\n  (f 5))\n```\n\n\nThis program will NOT produce a value.\nRecall that in a `let`, the scope of a left-hand-side variable includes\nnone of the right-hand-side expressions.\nSo, the var`lambda` expression can NOT refer to var`f`.\n\nIf we use `letrec` rather than `let`, this program will work.\n\nYou can even use `letrec` to define mutually recursive functions. For example,\n\n```\n(letrec ([od? (lambda (n)\n                (if (eq? n 0)\n                    #f\n                    (ev? (- n 1))))]\n         [ev? (lambda (n)\n                (if (eq? n 0)\n                    #t\n                    (od? (- n 1))))])\n  (od? 2))\n```\n\nLet's summarize the scoping rules of `letrec`.\n"
      },
      "goal_letrec": {
        "emphasize": "In a `letrec`, the scope of a left-hand-side variable includes\n*all* of the right-hand-side expressions.\n\nIf we put it visually,\n\n<image src=\"data:image/png;base64,\" />\n"
      },
      "intro_letstar": {
        "confirm": "The following program tries to use `let` to process var`x` through a series of operations.\n\n```\n(deffun (double n)\n  (* n 2))\n(deffun (add1 n)\n  (+ n 1))\n\n(let ([x 0]\n      [x (add1 x)]\n      [x (double x)])\n  (+ 3 x))\n```\n\n\nThis program will error because none of the right-hand-side expressions can refer to var`x`.\nReplacing `let` with `letrec` will NOT fix the error.\nRecall that in a `letrec`, the scope of a left-hand-side variable includes\n*all* of the right-hand-side expressions.\nTo achieve this while avoiding confusion,\n`letrec` insists that all left-hand-side variables must be distinct.\n(`let` also has this restriction, but for a different reason.)\n\nIf we replace `let` with `let*`, this program will produce `5`, the value of term`(+ 3 (double (add1 0)))`.\n`let*` allows you to do a series of bindings sequentially.\n\nLet's summarize the scoping rules of `let*`.\nAfter that, we will do a handful of exercises to test your understanding of `let`, `letrec`, and `let*`.\n"
      },
      "goal_letstar": {
        "emphasize": "In a `let*`, the scope of a left-hand-side variable includes *all of the subsequent* right-hand-side expressions.\n\nIf we put it visually,\n\n<image src=\"data:image/png;base64,\" />\n"
      },
      "swap_let": {
        "program": "(defvar x 1)\n(defvar y 2)\n\n(let ([x (+ y 1)]\n      [y (+ x 1)])\n  (mvec x y))\n",
        "answer": "#(3 2)",
        "feedback": "The top-level environment binds var`x` to `2` and var`y` to `3`.\n\nNeither term`(+ y 1)` nor term`(+ x 1)` refers to the var`x` and var`y` bound by the `let`. So, they both refer to the top-level definitions.\n",
        "misconceptions": {
          "#(3 4)": {
            "misconception": "Other"
          },
          "error": {
            "misconception": "Other"
          }
        },
        "again": {
          "program": "(defvar a 1)\n(defvar b 2)\n\n(let ([a (* b 10)]\n      [b (* a 10)])\n  (mvec a b))\n",
          "answer": "#(20 10)"
        }
      },
      "swap_letrec": {
        "program": "(defvar x 1)\n(defvar y 2)\n\n(letrec ([x (+ y 1)]\n         [y (+ x 1)])\n  (mvec x y))\n",
        "answer": "error",
        "feedback": "term`(+ y 1)` refers to the local var`y`, which is not bound to a value (but a `💣`) when term`(+ y 1)` is evaluated.\n",
        "misconceptions": {
          "#(3 2)": {
            "misconception": "Other"
          },
          "#(3 4)": {
            "misconception": "Other"
          }
        },
        "again": {
          "program": "(defvar a 1)\n(defvar b 2)\n\n(letrec ([a (* b 10)]\n         [b (* a 10)])\n  (mvec a b))\n",
          "answer": "error"
        }
      },
      "swap_letstar": {
        "program": "(defvar x 1)\n(defvar y 2)\n\n(let* ([x (+ y 1)]\n       [y (+ x 1)])\n  (mvec x y))\n",
        "answer": "#(3 4)",
        "feedback": "The top-level environment binds var`x` to `2` and var`y` to `3`.\n\nterm`(+ y 1)` refers to the top-level var`y` while term`(+ x 1)` refers to the local var`x`.\n",
        "misconceptions": {
          "#(3 2)": {
            "misconception": "Other"
          },
          "error": {
            "misconception": "Other"
          }
        },
        "again": {
          "program": "(defvar a 7)\n(defvar b 5)\n\n(let* ([a (+ b 10)]\n       [b (+ a 10)])\n  (mvec a b))\n",
          "answer": "#(15 25)"
        }
      }
    }
  },
  "post1": {
    "misconceptions": [],
    "order": [
      "intro_post1",
      [
        "post_new_three_layer",
        "post_error_when_refer_to_undefined",
        "post_bad_order"
      ]
    ],
    "questions": {
      "intro_post1": {
        "confirm": "In this tutorial, we will review previously learned content.\n"
      },
      "post_new_three_layer": {
        "program": "(defvar a 3)\n(deffun (foo b)\n  (deffun (bar)\n    (defvar c 6)\n    (+ a b c))\n  (bar))\n(+ (foo 4) 2)\n",
        "answer": "15",
        "feedback": "This program binds var`a` to `3` and var`foo` to a function, and then\nevaluates term`(+ (foo 4) 2)`.\nThe value of term`(+ (foo 4) 2)` is the value of term`(+ (bar) 2)`,\nwhich is the value of term`(+ (+ a b z) 2)`,\nwhich is the value of term`(+ (+ 3 4 6) 2)`,\nwhich is `10`.\n",
        "misconceptions": {
          "error": {
            "misconception": "IsolatedFun",
            "feedback": "You might think term`(+ a b c)` can't refer to var`a` and var`b`.\nHowever, term`(+ a b c)` appears in the body of var`bar`, which is a sub-block of\nthe body of var`foo`, which is a sub-block of the top-level block.\nSo, it can refer to the var`a` defined in the top-level block\nand the var`b` defined in var`foo`.\nIn SMoL, variable references follow the hierarchical structure of blocks.\n"
          }
        },
        "again": {
          "program": "(defvar aa 3)\n(deffun (abc bb)\n  (deffun (h)\n    (defvar cc 2)\n    (* aa bb cc))\n  (h))\n(* (abc 4) 1)\n",
          "answer": "24"
        }
      },
      "post_error_when_refer_to_undefined": {
        "program": "(deffun (k a)\n  (defvar b 1)\n  (+ a b))\n\n(+ (k 3) b)\n",
        "answer": "error",
        "feedback": "term`(+ (k 3) b)` is evaluated in the top-level block, where var`b` is not defined.\nSo, this program errors.\n",
        "misconceptions": {
          "5": {
            "misconception": "FlatEnv",
            "feedback": "term`(defvar b 1)` binds var`b` to `1`.\nYou might think the binding applies everywhere.\nHowever, it only applies to the body of var`k`.\nterm`(+ (k 3) b)` appears in the top-level block, so it tries to refer to\na var`b` defined in the top-level block. But var`b` is not defined in the top-level block.\nIn SMoL, variable references follow the hierarchical structure of blocks.\n"
          }
        },
        "again": {
          "program": "(deffun (foo bar)\n  (defvar zzz 8)\n  (* bar zzz))\n\n(* (foo 9) zzz)\n",
          "answer": "error"
        }
      },
      "post_bad_order": {
        "program": "(defvar i (* j 3))\n(defvar j 2)\n\ni\nj\n",
        "answer": "error",
        "feedback": "The first definition tries to bind var`i` to the value of term`(* j 3)`.\nTo evaluate term`(* j 3)`, we need the value of var`j`.\nBut var`j` is not bound to a value at that moment.\n",
        "misconceptions": {
          "6 2": {
            "misconception": "Lazy",
            "feedback": "You might think term`(* j 3)` is able to refer to var`j` and hence evaluate to a value.\nHowever, var`j` has not been bound to a value when term`(* j 3)` is evaluated.\nIn SMoL, every block evaluates its definitions and expressions in reading order (i.e., top-to-bottom and left-to-right).\n"
          }
        },
        "again": {
          "program": "(defvar baz (+ bar 1))\n(defvar bar 2)\n\nbaz\nbar\n",
          "answer": "error"
        }
      }
    }
  },
  "post2": {
    "misconceptions": [],
    "order": [
      "intro_post2",
      [
        "post_new_three_layer",
        "post_error_when_refer_to_undefined",
        "post_bad_order",
        "post_alias_with_defvar",
        "post_alias_with_funcall",
        "post_alias_mvec_in_mvec",
        "post_basic_circularity"
      ]
    ],
    "questions": {
      "intro_post2": {
        "confirm": "In this tutorial, we will review previously learned content.\n"
      },
      "post_new_three_layer": {
        "program": "(defvar n 5)\n(deffun (f1 m)\n  (deffun (f2)\n    (defvar l 4)\n    (+ n m l))\n  (f2))\n(+ (f1 1) 3)\n",
        "answer": "13",
        "feedback": "This program binds var`n` to `5` and var`f1` to a function, and then\nevaluates term`(+ (f1 1) 3)`.\nThe value of term`(+ (f1 1) 3)` is the value of term`(+ (f2) 3)`,\nwhich is the value of term`(+ (+ n m l) 3)`,\nwhich is the value of term`(+ (+ 5 1 4) 3)`,\nwhich is `13`.\n",
        "misconceptions": {
          "error": {
            "misconception": "IsolatedFun",
            "feedback": "You might think term`(+ n m l)` can't refer to var`n` and var`m`.\nHowever, term`(+ n m l)` appears in the body of var`f2`, which is a sub-block of\nthe body of var`f1`, which is a sub-block of the top-level block.\nSo, it can refer to the var`n` defined in the top-level block\nand the var`m` defined in var`f1`.\nIn SMoL, variable references follow the hierarchical structure of blocks.\n"
          }
        },
        "again": {
          "program": "(defvar aa 3)\n(deffun (abc bb)\n  (deffun (h)\n    (defvar cc 2)\n    (* aa bb cc))\n  (h))\n(* (abc 4) 1)\n",
          "answer": "24"
        }
      },
      "post_error_when_refer_to_undefined": {
        "program": "(deffun (foobar n)\n  (defvar m 4)\n  (+ n m))\n\n(+ (foobar 2) m)\n",
        "answer": "error",
        "feedback": "term`(+ (foobar 2) m)` is evaluated in the top-level block, where var`m` is not defined.\nSo, this program errors.\n",
        "misconceptions": {
          "10": {
            "misconception": "FlatEnv",
            "feedback": "term`(defvar m 4)` binds var`m` to `4`.\nYou might think the binding applies everywhere.\nHowever, it only applies to the body of var`foobar`.\nterm`(+ (foobar 2) m)` appears in the top-level block, so it tries to refer to\nan var`m` defined in the top-level block. But var`m` is not defined in the top-level block.\nIn SMoL, variable references follow the hierarchical structure of blocks.\n"
          }
        },
        "again": {
          "program": "(deffun (foo bar)\n  (defvar zzz 8)\n  (* bar zzz))\n\n(* (foo 9) zzz)\n",
          "answer": "error"
        }
      },
      "post_bad_order": {
        "program": "(defvar n (* m 4))\n(defvar m 3)\n\nn\nm\n",
        "answer": "error",
        "feedback": "The first definition tries to bind var`n` to the value of term`(* m 4)`.\nTo evaluate term`(* m 4)`, we need the value of var`m`.\nBut var`m` is not bound to a value at that moment.\n",
        "misconceptions": {
          "12 3": {
            "misconception": "Lazy",
            "feedback": "You might think term`(* m 4)` is able to refer to var`m` and hence evaluate to a value.\nHowever, var`m` has not been bound to a value when term`(* m 4)` is evaluated.\nIn SMoL, every block evaluates its definitions and expressions in reading order (i.e., top-to-bottom and left-to-right).\n"
          }
        },
        "again": {
          "program": "(defvar baz (+ bar 1))\n(defvar bar 2)\n\nbaz\nbar\n",
          "answer": "error"
        }
      },
      "post_alias_with_defvar": {
        "program": "(defvar v1 (mvec 23))\n(defvar v2 v1)\n(vec-set! v1 0 45)\nv2\n",
        "answer": "#(45)",
        "feedback": "The program creates a one-element vector (the only element being 23)\nand binds it to both var`v1` and var`v2`.\nThe `0`-th element of the vector is then replaced with `45`.\nSo, when the vector is printed as `#(45)`.\n",
        "misconceptions": {
          "#(23)": {
            "misconception": "DefCopyStruct",
            "feedback": "You might think var`v1` and var`v2` refer to different vectors, so changing var`v1` doesn't change var`v2`.\nHowever, they refer to the same vector.\nIn SMoL, a vector can be referred to by more than one variable, and binding a vector to a new variable does not create a copy of that vector.\n"
          }
        },
        "again": {
          "program": "(defvar a (mvec 37 26))\n(defvar b a)\n(vec-set! a 0 87)\nb\n",
          "answer": "#(87 26)"
        }
      },
      "post_alias_with_funcall": {
        "program": "(defvar m1 (mvec 77 77))\n(deffun (f m2)\n  (vec-set! m2 0 43))\n(f m1)\nm1\n",
        "answer": "#(43 77)",
        "feedback": "The program first creates a two-element vector. The elements are both `77`.\nAfter that, the program defines a function var`f`.\nThe function call term`(f m1)` replaces the first vector element with `43`.\nAfter that, the vector is printed. The vector now refers `43` and `77`, so the\nresult is `#(43 77)`.\n",
        "misconceptions": {
          "#(77 77)": {
            "misconception": "CallCopyStruct",
            "feedback": "You might think var`m1` and var`m2` refer to different vectors, so changing var`m2` doesn't change var`m1`.\nHowever, they refer to the same vector.\nIn SMoL, a vector can be referred to by more than one variable, and vectors that are passed to a function in a function call do not get copied..\n"
          }
        },
        "again": {
          "program": "(defvar a (mvec 55 17))\n(deffun (foobar b)\n  (vec-set! b 0 52))\n(foobar a)\na\n",
          "answer": "#(52 17)"
        }
      },
      "post_alias_mvec_in_mvec": {
        "program": "(defvar v1 (mvec 53))\n(defvar v2 (mvec 72 v1))\n(vec-set! v1 0 72)\nv2\n",
        "answer": "#(72 #(72))",
        "feedback": "`v1` is bound to a one-element vector.\nThe only element of this vector is `53`.\n`v2` is bound to a two-element vector.\nThe elements of this vector are `72` and the one-element vector.\nThe subsequent term`(vec-set! v1 0 72)` mutates the one-element vector.\nFinally, the value of var`v2` is printed.\n",
        "misconceptions": {
          "#(72 #(53))": {
            "misconception": "StructCopyStruct",
            "feedback": "You might think var`v1` and the 1-th element of var`v2` refer to different vectors, so changing var`v1` doesn't change the 1-th element of var`v2`.\nHowever, they refer to the same vector.\nIn SMoL, a vector can be referred to by multiple places, and creating new vectors that refer to existing ones does not create new copies of the existing vectors.\n"
          }
        },
        "again": {
          "program": "(defvar m (mvec 0))\n(defvar v (mvec 52 m 53))\n(vec-set! m 0 71)\nv\n",
          "answer": "#(52 #(71) 53)"
        }
      },
      "post_basic_circularity": {
        "program": "(defvar m (mvec 82 76))\n(vec-set! m 0 m)\n(vec-ref m 1)\n",
        "answer": "76",
        "feedback": "var`m` is bound to a vector.\nterm`(vec-set! m 0 m)` replaces the `0`-th element of the vector with the vector itself.\nThis is fine because a vector element can be any value, including itself.\nBesides, the vector is not copied, so the mutation finishes immediately.\n",
        "misconceptions": {
          "error": {
            "misconception": "NoCircularity",
            "feedback": "You might think var`m` can't refer to itself.\nHowever, this is not the case.\nIn SMoL, vectors can be referred to by arbitrary vectors, including themselves.\n"
          }
        },
        "again": {
          "program": "(defvar abc (mvec 84 82 85 86))\n(vec-set! abc 1 abc)\n(vec-ref abc 0)\n",
          "answer": "84"
        }
      }
    }
  },
  "post3": {
    "misconceptions": [
      "FunNotVal"
    ],
    "order": [
      "intro_post3",
      [
        "post_alias_with_defvar",
        "post_alias_with_funcall",
        "post_alias_mvec_in_mvec",
        "post_basic_circularity",
        "post_new_fun_rem_env",
        "post_not_aliased_by_defvar_1",
        "post_not_aliased_by_funarg_2",
        "post_new_alias_mvec_in_mvec_trick"
      ]
    ],
    "questions": {
      "intro_post3": {
        "confirm": "In this tutorial, we will review previously learned content.\n"
      },
      "post_alias_with_defvar": {
        "program": "(defvar p (mvec 72))\n(defvar q p)\n(vec-set! p 0 94)\nq\n",
        "answer": "#(94)",
        "feedback": "The program creates a one-element vector (the only element being 72)\nand binds it to both var`p` and var`q`.\nThe `0`-th element of the vector is then replaced with `94`.\nSo, when the vector is printed as `#(94)`.\n",
        "misconceptions": {
          "#(72)": {
            "misconception": "DefCopyStruct",
            "feedback": "You might think var`p` and var`q` refer to different vectors, so changing var`p` doesn't change var`q`.\nHowever, they refer to the same vector.\nIn SMoL, a vector can be referred to by more than one variable, and binding a vector to a new variable does not create a copy of that vector.\n"
          }
        },
        "again": {
          "program": "(defvar a (mvec 37 26))\n(defvar b a)\n(vec-set! a 0 87)\nb\n",
          "answer": "#(87 26)"
        }
      },
      "post_alias_with_funcall": {
        "program": "(defvar zz (mvec 88 88))\n(deffun (f aa)\n  (vec-set! aa 0 97))\n(f zz)\nzz\n",
        "answer": "#(97 88)",
        "feedback": "The program first creates a two-element vector. The elements are both `88`.\nAfter that, the program defines a function var`f`.\nThe function call term`(f zz)` replaces the first vector element with `97`.\nAfter that, the vector is printed. The vector now refers `97` and `88`, so the\nresult is `#(97 88)`.\n",
        "misconceptions": {
          "#(88 88)": {
            "misconception": "CallCopyStruct",
            "feedback": "You might think var`zz` and var`aa` refer to different vectors, so changing var`aa` doesn't change var`zz`.\nHowever, they refer to the same vector.\nIn SMoL, a vector can be referred to by more than one variable, and vectors that are passed to a function in a function call do not get copied..\n"
          }
        },
        "again": {
          "program": "(defvar a (mvec 55 17))\n(deffun (foobar b)\n  (vec-set! b 0 52))\n(foobar a)\na\n",
          "answer": "#(52 17)"
        }
      },
      "post_alias_mvec_in_mvec": {
        "program": "(defvar nn (mvec 66))\n(defvar mm (mvec 77 nn))\n(vec-set! nn 0 77)\nmm\n",
        "answer": "#(77 #(77))",
        "feedback": "var`nn` is bound to a one-element vector.\nThe only element of this vector is `66`.\nvar`mm` is bound to a two-element vector.\nThe elements of this vector are `77` and the one-element vector.\nThe subsequent term`(vec-set! nn 0 77)` mutates the one-element vector.\nFinally, the value of var`mm` is printed.\n",
        "misconceptions": {
          "#(77 #(66))": {
            "misconception": "StructCopyStruct",
            "feedback": "You might think var`nn` and the 1-th element of var`mm` refer to different vectors, so changing var`nn` doesn't change the 1-th element of var`mm`.\nHowever, they refer to the same vector.\nIn SMoL, a vector can be referred to by multiple places, and creating new vectors that refer to existing ones does not create new copies of the existing vectors.\n"
          }
        },
        "again": {
          "program": "(defvar m (mvec 0))\n(defvar v (mvec 52 m 53))\n(vec-set! m 0 71)\nv\n",
          "answer": "#(52 #(71) 53)"
        }
      },
      "post_basic_circularity": {
        "program": "(defvar ns (mvec 74 85))\n(vec-set! ns 0 ns)\n(vec-ref ns 1)\n",
        "answer": "85",
        "feedback": "var`ns` is bound to a vector.\nterm`(vec-set! ns 0 ns)` replaces the `0`-th element of the vector with the vector itself.\nThis is fine because a vector element can be any value, including itself.\nBesides, the vector is not copied, so the mutation finishes immediately.\n",
        "misconceptions": {
          "error": {
            "misconception": "NoCircularity",
            "feedback": "You might think var`ns` can't refer to itself.\nHowever, this is not the case.\nIn SMoL, vectors can be referred to by arbitrary vectors, including themselves.\n"
          }
        },
        "again": {
          "program": "(defvar abc (mvec 84 82 85 86))\n(vec-set! abc 1 abc)\n(vec-ref abc 0)\n",
          "answer": "84"
        }
      },
      "post_new_fun_rem_env": {
        "program": "(defvar n 7)\n(deffun (foo)\n  n)\n(deffun (bar)\n  (set! n 3)\n  (foo))\n(bar)\n(set! n 5)\n(foo)\n",
        "answer": "3 5",
        "feedback": "There is exactly one variable var`n`.\nThe var`n` in term`(set! n 3)` refers to that variable.\nCalling var`bar` evaluates term`(set! n 3)`, which mutates var`n`.\nWhen the value of var`n` is eventually printed, we see the new value.\n",
        "misconceptions": {
          "7 5": {
            "misconception": "DefOrSet",
            "feedback": "You might think term`(set! n 3)` defines var`n`.\nHowever, it will mutate the top-level var`n` defined outside the functions.\nIn SMoL, variable assignments mutate existing bindings and never create new bindings.\n"
          },
          "7 7": {
            "misconception": "DeepClosure",
            "feedback": "You are right that var`n` is bound to `7` when var`f` is bound to a function.\nYou might think the function remembers the value `7`.\nHowever, var`f` does not remember the value of var`n`. Rather, it always\nrefers to the latest value of var`n`.\nIn SMoL, functions refer to the latest values of variables defined outside their definitions.\n"
          },
          "error": {
            "misconception": "IsolatedFun",
            "feedback": "You might think var`foo` can't refer to var`n`.\nHowever, var`foo` is defined in the top-level block, so its body is allowed to refer to the top-level var`n`.\n\nYou might think term`(set! n 3)` can't mutate var`n`.\nHowever, term`(set! n 3)` appears in the body of var`bar`, which is a sub-block of the top-level block,\nso it is allowed to mutate the top-level var`n`.\n\nIn SMoL, variable references follow the hierarchical structure of blocks.\n"
          }
        },
        "again": {
          "program": "(defvar a 4)\n(deffun (h)\n  a)\n(deffun (k)\n  (set! a 2)\n  (h))\n(k)\n(set! a 6)\n(h)\n",
          "answer": "2 6"
        }
      },
      "post_not_aliased_by_defvar_1": {
        "program": "(defvar a 6)\n(defvar b a)\n(set! a 5)\na\nb\n",
        "answer": "5 6",
        "feedback": "The first definition binds var`a` to `6`.\nThe second definition binds var`b` to the value of var`a`, which is `6`.\nThe `set!` mutates the binding of var`a`, so var`a` is now bound to `5`.\nBut var`b` is still bound to `6`.\n",
        "misconceptions": {
          "5 5": {
            "misconception": "DefByRef",
            "feedback": "You might think term`(defvar b a)` binds var`b` to var`a`, so changing var`a` will change var`b`.\nHowever, we learned in previous tutorials that variables are bound to values, so var`b` is bound to the `5`, i.e., *the value of* var`a`.\nIn SMoL, variable assignments change *only* the mutated variables.\n"
          }
        },
        "again": {
          "program": "(defvar a 1)\n(defvar b a)\n(set! a 2)\na\nb\n",
          "answer": "2 1"
        }
      },
      "post_not_aliased_by_funarg_2": {
        "program": "(defvar a 5)\n(deffun (k b)\n  (set! b 3)\n  a)\n(k a)\n",
        "answer": "5",
        "feedback": "The function call binds var`b` to `5`.\nThe `set!` mutates the value of var`b` to `3`, but var`a` is still bound to `5`.\n",
        "misconceptions": {
          "3": {
            "misconception": "CallByRef",
            "feedback": "You might think the function call term`(k a)` binds var`b` to var`a`, so\nchanging var`b` will change var`a`.\nHowever, we learned in previous tutorials that variables are bound to values, so var`b` is bound to `5`, i.e., *the value of* var`a`.\nIn SMoL, variable assignments change *only* the mutated variables.\n"
          },
          "error": {
            "misconception": "IsolatedFun",
            "feedback": "You might think var`k` can't refer to var`a`.\nHowever, var`k` is defined in the top-level block,\nso its body can refer to the top-level var`a`.\nIn SMoL, variable references follow the hierarchical structure of blocks.\n"
          }
        },
        "again": {
          "program": "(defvar s 1)\n(deffun (foobar t)\n  (set! t 2)\n  s)\n(foobar s)\n",
          "answer": "1"
        }
      },
      "post_new_alias_mvec_in_mvec_trick": {
        "program": "(defvar m (mvec 66))\n(defvar z (mvec m 66 66))\n(set! m (mvec 43))\nz\n",
        "answer": "#(#(66) 66 66)",
        "feedback": "var`m` is first bound to a one-element vector.\nvar`z` is bound to a three-element vector, of which the first element is the one-element vector.\nterm`(set! m (mvec 43))` binds var`m` to a new vector.\nThis doesn't impact var`z` because\nthe `0`-th element of var`z` is still the one-element vector.\n",
        "misconceptions": {
          "#(#(43) 66 66)": {
            "misconception": "StructByRef",
            "feedback": "You might think the `0`-th and first element of var`z` refers to var`m`.\nHowever, vectors refer to values, so it actually refers to the one-element vector, i.e., *the value of* var`m`.\nIn SMoL, variable assignments change *only* the mutated variables.\n"
          }
        },
        "again": {
          "program": "(defvar x (mvec 0))\n(defvar v (mvec 2 x 3))\n(set! x (mvec 1))\nv\n",
          "answer": "#(2 #(0) 3)"
        }
      }
    }
  },
  "post4": {
    "misconceptions": [
      "Other"
    ],
    "order": [
      "intro_post4",
      [
        "post_new_fun_rem_env",
        "post_not_aliased_by_defvar_1",
        "post_not_aliased_by_funarg_2",
        "post_new_alias_mvec_in_mvec_trick",
        "post_lambda_fun_ref_env",
        "post_lambda_new_state"
      ]
    ],
    "questions": {
      "intro_post4": {
        "confirm": "In this tutorial, we will review previously learned content.\n"
      },
      "post_new_fun_rem_env": {
        "program": "(defvar t 6)\n(deffun (f1)\n  t)\n(deffun (f2)\n  (set! t 4)\n  (f1))\n(f2)\n(set! t 2)\n(f1)\n",
        "answer": "4 2",
        "feedback": "There is exactly one variable var`t`.\nThe var`t` in term`(set! t 4)` refers to that variable.\nCalling var`f2` evaluates term`(set! t 4)`, which mutates var`t`.\nWhen the value of var`t` is eventually printed, we see the new value.\n",
        "misconceptions": {
          "6 2": {
            "misconception": "DefOrSet",
            "feedback": "You might think term`(set! t 4)` defines var`t`.\nHowever, it will mutate the top-level var`t` defined outside the functions.\nIn SMoL, variable assignments mutate existing bindings and never create new bindings.\n"
          },
          "6 6": {
            "misconception": "DeepClosure",
            "feedback": "You are right that var`t` is bound to `6` when var`f` is bound to a function.\nYou might think the function remembers the value `6`.\nHowever, var`f` does not remember the value of var`t`. Rather, it always\nrefers to the latest value of var`t`.\nIn SMoL, functions refer to the latest values of variables defined outside their definitions.\n"
          },
          "error": {
            "misconception": "IsolatedFun",
            "feedback": "You might think var`f1` can't refer to var`t`.\nHowever, var`f1` is defined in the top-level block, so its body is allowed to refer to the top-level var`t`.\n\nYou might think term`(set! t 4)` can't mutate var`t`.\nHowever, term`(set! t 4)` appears in the body of var`f2`, which is a sub-block of the top-level block,\nso it is allowed to mutate the top-level var`t`.\n\nIn SMoL, variable references follow the hierarchical structure of blocks.\n"
          }
        },
        "again": {
          "program": "(defvar a 4)\n(deffun (h)\n  a)\n(deffun (k)\n  (set! a 2)\n  (h))\n(k)\n(set! a 6)\n(h)\n",
          "answer": "2 6"
        }
      },
      "post_not_aliased_by_defvar_1": {
        "program": "(defvar n 3)\n(defvar m n)\n(set! n 6)\nn\nm\n",
        "answer": "6 3",
        "feedback": "The first definition binds var`n` to `3`.\nThe second definition binds var`m` to the value of var`n`, which is `3`.\nThe `set!` mutates the binding of var`n`, so var`n` is now bound to `6`.\nBut var`m` is still bound to `3`.\n",
        "misconceptions": {
          "6 6": {
            "misconception": "DefByRef",
            "feedback": "You might think term`(defvar m n)` binds var`m` to var`n`, so changing var`n` will change var`m`.\nHowever, we learned in previous tutorials that variables are bound to values, so var`m` is bound to the `6`, i.e., *the value of* var`n`.\nIn SMoL, variable assignments change *only* the mutated variables.\n"
          }
        },
        "again": {
          "program": "(defvar a 1)\n(defvar b a)\n(set! a 2)\na\nb\n",
          "answer": "2 1"
        }
      },
      "post_not_aliased_by_funarg_2": {
        "program": "(defvar n 2)\n(deffun (h m)\n  (set! m 7)\n  n)\n(h n)\n",
        "answer": "2",
        "feedback": "The function call binds var`m` to `2`.\nThe `set!` mutates the value of var`m` to `7`, but var`n` is still bound to `2`.\n",
        "misconceptions": {
          "7": {
            "misconception": "CallByRef",
            "feedback": "You might think the function call term`(h n)` binds var`m` to var`n`, so\nchanging var`m` will change var`n`.\nHowever, we learned in previous tutorials that variables are bound to values, so var`m` is bound to `2`, i.e., *the value of* var`n`.\nIn SMoL, variable assignments change *only* the mutated variables.\n"
          },
          "error": {
            "misconception": "IsolatedFun",
            "feedback": "You might think var`h` can't refer to var`n`.\nHowever, var`h` is defined in the top-level block,\nso its body can refer to the top-level var`n`.\n\nIn SMoL, variable references follow the hierarchical structure of blocks.\n"
          }
        },
        "again": {
          "program": "(defvar s 1)\n(deffun (foobar t)\n  (set! t 2)\n  s)\n(foobar s)\n",
          "answer": "1"
        }
      },
      "post_new_alias_mvec_in_mvec_trick": {
        "program": "(defvar a (mvec 88))\n(defvar c (mvec a 88 88))\n(set! a (mvec 76))\nc\n",
        "answer": "#(#(88) 88 88)",
        "feedback": "var`a` is first bound to a one-element vector.\nvar`c` is bound to a three-element vector, of which the first element is the one-element vector.\nterm`(set! a (mvec 76))` binds var`a` to a new vector.\nThis doesn't impact var`c` because\nthe `0`-th element of var`c` is still the one-element vector.\n",
        "misconceptions": {
          "#(#(76) 88 88)": {
            "misconception": "StructByRef",
            "feedback": "You might think the `0`-th and first element of var`c` refers to var`a`.\nHowever, vectors refer to values, so it actually refers to the one-element vector, i.e., *the value of* var`a`.\nIn SMoL, variable assignments change *only* the mutated variables.\n"
          }
        },
        "again": {
          "program": "(defvar x (mvec 0))\n(defvar v (mvec 2 x 3))\n(set! x (mvec 1))\nv\n",
          "answer": "#(2 #(0) 3)"
        }
      },
      "post_lambda_fun_ref_env": {
        "program": "(deffun (k b)\n  (lambda (a)\n    (+ a b)))\n(defvar foo (k 3))\n(defvar bar (k 2))\n(foo 3)\n(bar 3)\n",
        "answer": "6 5",
        "feedback": "The value of term`(k 3)` is a lambda function defined in an\nenvironment where var`b` is bound to `3`.\nThe value of term`(k 2)` is *another* lambda function defined in an\nenvironment where var`b` is bound to `2`.\nThe two lambda functions are *different* values.\nSo, the value of term`(foo 3)` is `6`,\nwhile the value of term`(bar 3)` is `5`.\n",
        "misconceptions": {
          "5 5": {
            "misconception": "FlatEnv",
            "feedback": "var`k` is called twice.\nThe first call binds var`a` to `3`.\nThe second call binds var`a` to `2`.\nYou might think the second binding applies everywhere.\nHowever, it only applies to the second call.\nThe lambda function that var`foo` refers to is created in the first call, so term`(foo 3)` adds `3` rather than `2`.\nIn SMoL, variable references follow the hierarchical structure of blocks.\n"
          },
          "error": {
            "misconception": "FunNotVal,IsolatedFun",
            "feedback": "You might think that term`(k 3)` and term`(k 2)` errors because they return functions,\nand that they would not error if they return values of other kinds (e.g., numbers and vectors).\nHowever, they do not error.\n\nYou might think that the two variable definitions error because the variables are bound to functions,\nand that they would not error if the variables were bound to values of other kinds (e.g., numbers and vectors).\nHowever, they do not error.\n\nIn SMoL, functions are (also) *first-class* citizens of the value world.\n"
          }
        },
        "again": {
          "program": "(deffun (f n)\n  (lambda (m)\n    (* m n)))\n(defvar fun1 (f 10))\n(defvar fun2 (f 1))\n(fun1 4)\n(fun2 4)\n",
          "answer": "40 4"
        }
      },
      "post_lambda_new_state": {
        "program": "(defvar a 1)\n(deffun (foo)\n  (lambda (b)\n    (+ a b)))\n(defvar bar (foo))\n(set! a 3)\n(bar 0)\n",
        "answer": "3",
        "feedback": "var`a` is bound to `1`.\nvar`bar` is bound to the lambda function.\nterm`(set! a 3)` binds var`a` to `3`.\nSo, the value of term`(bar 0)` is the value of term`(+ 3 0)`, which is `3`.\n",
        "misconceptions": {
          "1": {
            "misconception": "DeepClosure",
            "feedback": "You are right that var`a` is bound to `1` when the lambda function is created.\nYou might think the function remembers the value `1`.\nHowever, the function does not remember the value of var`a`.  Rather, it remembers the environment and hence always refers to the latest value of var`a`.\nIn SMoL, functions refer to the latest values of variables defined outside their definitions.\n"
          },
          "error": {
            "misconception": "FunNotVal,IsolatedFun",
            "feedback": "You might think that term`(foo)` errors because it returns a function,\nand that it would not error if it returns a value of other kinds (e.g., numbers and vectors).\nHowever, it does not error.\n\nYou might think that term`(defvar bar (foo))` errors because var`g` is bound to a function,\nand that it would not error if var`g` is bound to a value of other kinds (e.g., numbers and vectors).\nHowever, it does not error.\n\nIn SMoL, functions are (also) *first-class* citizens of the value world.\n"
          }
        },
        "again": {
          "program": "(defvar a 2)\n(deffun (make)\n  (lambda (b)\n    (+ a b)))\n(defvar fun (make))\n(set! a 1)\n(fun 1)\n",
          "answer": "2"
        }
      }
    }
  }
}